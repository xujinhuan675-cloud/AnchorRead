...
`Authorization: Bearer` 访问 `/mcp`。服务器会为用户确认的浏览器工作区签发不透明 `bindingId`，access token、授权码和
...
`revoke-authorizations` 会撤销该浏览器工作区下的 access-token 哈希和 OAuth refresh-token，
...
客户端只需要 MCP 地址。首次连接会自动打开浏览器授权，不需要复制 Token 或设置 Token 环境变量。授权归属于当前浏览器工作区，同源标签页和后续新开的图解页共享授权；关闭完成授权的临时页面不会撤销授权。
...
OAuth 客户端在协议内部通过 `Authorization: Bearer <access-token>` 认证。每次请求都会校验 token 的 `bindingId` 是否仍属于
服务器记录的浏览器工作区，再解析该工作区当前持有租约的在线图解标签页；MCP session 固定绑定 access token，不能在同一个

...
### 📖 原文阅读器：事实源永远不被修改
- 原文为唯一底稿，**解读 / 图解 / 重点 / 白话**都是可多选叠加的辅助层：选什么叠什么，全关即纯原文；白话会用易懂表述替换原文难点，并保留来源映射标记
- Markdown 全文渲染，支持表格、代码块，阅读进度自动保存

### 🧠 AI 行间解读与白话生成
- 选中任意句子"解释这段"，解读卡片贴着原文行内展开，可标记"懂了"或删除
- 选中难懂表述“生成白话”，生成白话卡并可一键定位回原文锚点
- **自维护术语表**：预先定义术语与释义（支持别名），解读与全文分析会把它作为背景告知 AI：表中术语不再重复解释，并沿用你的既定定义
- 重点与解读生成：一次分析按文章/段落/句子/词语分层标注原文，并在重点旁生成行间解读

### 🗺️ 文档关系图（Mermaid / Excalidraw）
- 选中原文锚定后生成文档关系图，**内联嵌入对应原文下方**，不再跳转
- 卡片内可切换 Mermaid / Excalidraw 双引擎，支持"查看源码"折叠、AI 优化图解代码
- 图解历史可回溯复用

### 🎯 按需辅助：你来决定页面铺什么
- 工具栏提供**解读 / 图解 / 白话**多选开关，**重点**可按层级选择叠加显示
- 所有功能围绕"快速理解这篇文档"服务，不把内容一股脑铺在页面上

### 🃏 闪卡（FSRS 间隔重复复习）
- 工具栏一键为当前文档生成闪卡，直接进入知识面板的"闪卡"页签
- FSRS-5 算法自动安排下次复习时间，支持翻卡评分（忘记/困难/记得/轻松）与跳过
- 卡片库按文档归档，可逐张删除；到期数量实时角标提醒

### 🔒 本地优先，数据自主
- 文档、解读、白话、图解、闪卡全部存于浏览器（IndexedDB / localStorage）
...
- **多种导出**：`.anchorread` 工作区备份、**Anki** 闪卡文本、**Obsidian** Markdown 笔记包（解读/白话）
...
- **MCP Server**：把导出的工作区只读暴露给 Claude 等 MCP 客户端（文档/白话/闪卡查询与全文搜索），详见 [docs/mcp-server.md](docs/mcp-server.md)
...
之后即可在阅读中生成解读、白话、关系图与闪卡。

### 阅读工作流

1. **导入文档**：粘贴正文、上传 `.md/.txt/.epub` 文件、粘贴网页 URL，或用浏览器扩展一键剪藏，解析并进入阅读
2. **读与懂**：选中句子“解释这段”、“生成白话”、“图解”锚定生成关系图，或执行你的自定义动作
3. **选**：用工具栏开关决定显示哪些内联辅助
4. **记**：点击"生成闪卡"，在知识面板"闪卡"页签里按间隔重复巩固；可导出 Anki / Obsidian 到外部生态
...
**Q: 解读会修改我的原文吗？**
A: 不会。解读、白话、图解、闪卡都是派生内容，随时可开关与删除，源文档保持不变。“白话”模式也只是生成替换视图，不改写原文。

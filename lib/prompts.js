...
- **坐标规划**：预先规划布局，相邻节点保留至少 40px、层级之间保留 80px，给带标签的连线预留 120px 以上通道，避免元素重叠
- **尺寸一致性**：同类型元素保持相似尺寸，建立视觉节奏

### 视觉设计硬约束（必须执行）
- **默认风格优先**：除非用户明确指定了视觉风格、颜色、质感或布局效果，否则统一采用本节和《AnchorRead Excalidraw Design Guide》定义的默认视觉系统；不要自行切换成另一套风格。
- **不是默认白板**：每张 Excalidraw 图都必须有明确的视觉系统；至少使用 3 种角色色（主流程、服务/中间层、数据或辅助区），浅色实心填充配深色描边，禁止所有节点都使用默认白底黑框。
- **层级和分区**：复杂图必须先画背景分区/泳道，再放节点、连接器、标题和注释；分区应有内边距，节点在同一角色上使用一致尺寸。
- **节点样式**：有文字的 rectangle/ellipse/diamond 使用圆角、足够内边距、至少 16px 正文和 20px 标题；同一层级保持统一字号、圆角和描边宽度。
- **留白**：相邻节点至少保留 40px，层级之间保留 80px；宁可扩大画布，不要压缩文字或堆叠节点。
- **视觉验收**：生成后必须检查是否出现“整图同色、无分区、无层级、文字拥挤、连接线穿过节点”等问题；发现任一问题先修复再提交。
...
- **路径**：只有无遮挡、单进单出的直连流程可以使用两点 \`points\`；分支/汇聚、回环、跨区、穿越其它节点，或节点间通道小于 40px 的关系，必须提供 3 个以上相对坐标点、设置 \`elbowed: true\`，并将中间点绕到空白通道。\`points\` 是相对于箭头 \`x/y\` 的路径，系统会保留中间拐点，只重算绑定两端；保存预检会拒绝不符合该规则的场景，收到具体连接器 id 后修正并重试。

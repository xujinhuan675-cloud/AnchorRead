import { assertDiagramBrowserWritable } from './diagram-browser-write-guard.js';
import { cloneForTransport } from './cloneable.js';

export const WORKSPACE_DB_NAME = 'anchor-read-workspace';
// Opening v4 closes v3 connections through IndexedDB's versionchange event.
// This fences pages from builds that predate the browser write guard.
...
function cloneValue(value) {
  return cloneForTransport(value);
}

function assertExpectedRevision(storeName, existing, expectedRevision) {
  if (storeName !== 'drawings' || expectedRevision === undefined || expectedRevision === null) return;
  const actualRevision = Number(existing?.revision || 0);
  if (actualRevision === Number(expectedRevision)) return;
  const error = new Error(`Diagram revision conflict: expected ${expectedRevision}, actual ${actualRevision}.`);
...
function assertStoreWritable(storeName, options = {}) {
  if (storeName === 'drawings' && options.allowUnpairedUi !== true) {
    assertDiagramBrowserWritable();
  }
}
...
export function createIndexedDbWorkspaceAdapter({
  ...
  return {
    ...
    put(storeName, record, options = {}) {
      return withStore(storeName, 'readwrite', async (store) => {
        assertStoreWritable(storeName, options);
        const existing = await requestToPromise(store.get(record.id));
        assertExpectedRevision(storeName, existing, options.expectedRevision);
...
export function createMemoryWorkspaceAdapter(initialState = {}) {
  ...
  return {
    ...
    async put(storeName, record, options = {}) {
      assertStoreWritable(storeName, options);
      assertExpectedRevision(storeName, getStore(storeName).get(record.id), options.expectedRevision);
...
function createCollectionRepository(storeName, adapter, normalizeOptions) {
  return Object.freeze({
    ...
    save: (record, options) => adapter.put(
      storeName,

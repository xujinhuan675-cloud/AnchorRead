 ...
 */
export function generateImagePrompt(chartType, engine = 'excalidraw') {
  const chartTypeText = chartType && chartType !== 'auto'
    ? `请将图片内容转换为${getChartTypeName(chartType)}类型的${engine === 'mermaid' ? 'Mermaid' : 'Excalidraw'}图表。`
    : `请分析图片内容并选择合适的图表类型转换为${engine === 'mermaid' ? 'Mermaid' : 'Excalidraw'}图表。`;
  const relationshipGuidance = engine === 'excalidraw'
    ? [
      '- 对流程图中的每条箭头/连线提供稳定 id，并设置 startElementId、endElementId（或 Excalidraw startBinding/endBinding）指向两端节点。',
      '- 将箭头上的关系谓词放入 arrow.label.text；节点文字放入 shape.label，不要用脱离容器的文字替代关系。',
      '- 让节点和连线的顺序支持自然演示：先出现起点节点，再出现终点节点，最后出现箭头及其关系文字；已出现的节点不要重复占一步。',
    ].join('\n')
    : '- Mermaid 流程关系应保留清晰的起点、终点和连接谓词，便于播放脚本按语义阶段组织。';
...
基于分析结果，创建清晰、准确的${engine === 'mermaid' ? 'Mermaid' : 'Excalidraw'}图表，确保：
- 保留图片中的所有关键信息
- 使用合适的图表类型展示内容
- 保持逻辑关系和结构
- 添加必要的文字说明
- 播放演示面向人观看，按语义关系组织出现顺序，避免镜头频繁追逐元素
${relationshipGuidance}

将图片里的内容转换为${engine === 'mermaid' ? 'Mermaid' : 'Excalidraw'}`;
}

...
import {
  ...
  normalizeDiagramAgentBuildInfo,
...
const DEFAULT_CONNECTION_TTL_MS = 5 * 60_000;
...
function publicConnection(connection, now = Date.now(), clientId = '') {
  ...
  const online = connection.expiresAt > now;
  const compatibility = diagramAgentBuildCompatibility(connection);
  const connected = online && compatibility.compatible;
  return {
    ...
    connected,
    writable: connected,
    buildSha: compatibility.actual.buildSha,
    buildVersion: compatibility.actual.buildVersion,
    protocolVersion: compatibility.actual.protocolVersion,
...
export class InMemoryDiagramMcpPairingStore {
  ...
  activeConnection(workspaceId, now = Date.now()) {
    return [...this.connections.values()]
      .filter((connection) => (
        ...
        && diagramAgentBuildCompatibility(connection).compatible
      ))
  ...
  workspaceBinding(workspace, bindingId = '') {
    const normalizedBindingId = String(bindingId || '').trim();
    const bindings = Object.values(workspace?.bindings || {})
    ...
    if (!normalizedBindingId) return bindings[0] || null;
    return bindings.find((binding) => binding.bindingId === normalizedBindingId) || null;
  }
  ...
  authorizedBinding(context) {
    ...
    const recorded = this.workspaceBinding(workspace, bindingId);
    if (!recorded) {
      throw storeError(
        'BROWSER_BINDING_MISMATCH',
        'The MCP authorization does not match this browser workspace. Complete browser authorization again.',
      );
    }
    const browserSessionId = identifier(context?.browserSessionId || recorded.browserSessionId, 'browserSessionId');
    return { workspace, workspaceId, browserSessionId, bindingId };
  }

  async ensureBrowserBinding(context, { now = Date.now() } = {}) {
    this.prune(now);
    const workspaceId = identifier(context?.workspaceId, 'workspaceId');
    const browserSessionId = identifier(context?.browserSessionId, 'browserSessionId');
    const managementSecret = identifier(context?.managementSecret, 'managementSecret');
    ...
    if (active) return publicConnection(active, now, context?.clientId);

    const existingBinding = this.workspaceBinding(workspace);
    ...
    if (!existingBinding) {
      bindings[browserSessionId] = {
        bindingId,
        browserSessionId,
        ...normalizeDiagramAgentBuildInfo(context),
        createdAt: now,
        lastSeenAt: now,
      };
    ...
    return {
      bindingId,
      workspaceId,
      browserSessionId: existingBinding?.browserSessionId || browserSessionId,
      tabId: '',
      clientId: '',
      connected: false,
      online: false,
  ...
  async registerConnection(context, { replace = false, now = Date.now() } = {}) {
    ...
    const bindingId = existingBinding?.bindingId
      || active?.bindingId
      || this.workspaceBinding(workspace)?.bindingId
      || randomId('binding');
    bindings[browserSessionId] = {
      bindingId,
      browserSessionId,
    ...
    const connection = {
      ...
      // A deliberate replacement starts a new connection generation even for
      // the same browser client. This fences delayed pagehide disconnects
      // from tearing down the freshly reconnected lease.
      generation: (existing?.generation || 0) + (existing?.clientId === clientId && !replace ? 0 : 1),
    };
  ...
  async disconnectConnection(context, { now = Date.now() } = {}) {
    ...
    const expectedGeneration = Number(context?.generation);
    if (Number.isFinite(expectedGeneration) && expectedGeneration !== connection.generation) return false;
    connection.expiresAt = now;
    connection.lastSeenAt = now;
  ...
  async assertConnectionOwner(context, { now = Date.now(), requireConnected = true } = {}) {
    ...
    if (requireConnected && !status.online) throw storeError('BROWSER_SESSION_OFFLINE', 'The paired browser workspace is offline.');
    if (!status.versionCompatible) {
      throw createBrowserBuildStaleError(status.actual, {
        expected: status.expected,
        workspaceUrl: status.href,
      });
    }
    return status;
  }
  ...
  getBindingInfo(context, { now = Date.now() } = {}) {
    ...
    const recorded = workspace.bindings?.[browserSessionId]
      || this.workspaceBinding(workspace, active?.bindingId)
      || this.workspaceBinding(workspace);
    const bindingId = String(recorded?.bindingId || '').trim();
    if (!bindingId) {
      throw storeError(
        'BROWSER_BINDING_MISMATCH',
        'This browser workspace is not paired with a server-issued binding. Complete browser authorization again.',
      );
    }
    const connection = active || this.connectionForBinding(workspaceId, bindingId);
    return {
      workspaceId,
      browserSessionId,
      bindingId,
  ...
  async revokeTokensForBinding(context, { clientId = '', tokenId = '', now = Date.now() } = {}) {
    ...
    for (const [id, token] of this.tokens) {
      ...
      revoked.push(publicToken({ ...token, revokedAt: now }, now));
      this.tokens.delete(id);
      this.tokenHashes.delete(token.tokenHash);
    }
    if (normalizedTokenId && revoked.length === 0) {
      throw storeError('TOKEN_NOT_FOUND', 'The requested MCP authorization was not found for this browser workspace.');
    }
  ...
  // `kind: personal` is the deliberately simple single-user alternative: it
  // is issued once by the same-origin browser and may be long-lived. The
  // token secret is returned only by this creation call.
  async createTokenForWorkspace(context, {
    name = 'MCP client',
    clientId = '',
    kind = 'oauth_access',
    ...
    const token = {
      id,
      kind: kind === 'personal' ? 'personal' : 'oauth_access',
  ...
  async createPersonalTokenForWorkspace(context, {
    ...
    this.prune(now);
    const binding = this.getBindingInfo(context, { now });
    // Keep one personal credential per browser workspace. Reissuing removes
    // the previous secret from both indexes so it cannot remain in the
    // effective authorization list after rotation.
    for (const [id, token] of this.tokens) {
      if (token.workspaceId === binding.workspaceId && token.kind === 'personal' && !token.revokedAt) {
        this.tokens.delete(id);
        this.tokenHashes.delete(token.tokenHash);
      }
    }
    // Call the base implementation directly so the file-backed subclass can
    // wrap this whole revoke+issue operation in one mutation queue without a
    // nested runMutation deadlock.
    return InMemoryDiagramMcpPairingStore.prototype.createTokenForWorkspace.call(this, {
      ...context,
  ...
  async authenticateToken(secret, { now = Date.now() } = {}) {
    ...
    const workspace = this.workspaces.get(token.workspaceId);
    if (!workspace || !this.workspaceBinding(workspace, token.bindingId)) {
      throw storeError(
        'BROWSER_BINDING_MISMATCH',
        'This MCP authorization belongs to a different browser workspace. Complete browser authorization again.',
      );
    }
    const active = this.activeConnection(token.workspaceId, now);
    const connection = active || this.connectionForBinding(token.workspaceId, token.bindingId);
...
export class FileDiagramMcpPairingStore extends InMemoryDiagramMcpPairingStore {
  ...
  async load() {
    ...
    try {
      ...
      for (const token of data.tokens || []) {
        // Non-expiring records are accepted only when explicitly marked as a
        // Personal Token. Legacy static tokens had no kind and remain
        // rejected during load.
        const kind = token?.kind === 'personal' ? 'personal' : 'oauth_access';
        const hasValidExpiry = Number.isFinite(Number(token?.expiresAt)) && Number(token.expiresAt) > 0;
        if (!token?.id || !token?.workspaceId || !token?.tokenHash || (!hasValidExpiry && kind !== 'personal') || !workspaces.has(token.workspaceId)) continue;
        tokens.set(token.id, { ...token, kind });
  ...
  ensureBrowserBinding(context, options) {
    return this.runMutation(() => super.ensureBrowserBinding(context, options));
  }
  ...
  createPersonalTokenForWorkspace(context, options) {
    return this.runMutation(() => super.createPersonalTokenForWorkspace(context, options));
  }

  ...
  async load() {
    ...
    try {
      ...
      for (const token of data.tokens || []) {
        ...
        if (!token?.id || !token?.workspaceId || !token?.tokenHash || !Number.isFinite(Number(token.expiresAt)) || Number(token.expiresAt) <= 0 || !this.workspaces.has(token.workspaceId)) continue;
        this.tokens.set(token.id, { ...token });
        this.tokenHashes.set(token.tokenHash, token.id);
      }
    } catch (error) {

...
export class InMemoryDiagramMcpPairingStore {
  ...
  async createPersonalTokenForWorkspace(context, {
    ...
    this.prune(now);
    const binding = this.getBindingInfo(context, { now });
    // Keep one active personal credential per browser workspace. Reissuing
    // intentionally revokes the previous secret so a lost copy can be
    // replaced without accumulating long-lived credentials.
    for (const token of this.tokens.values()) {

...
import { resolveTaskConfig } from '@/lib/task-routing';
 ...
 * 随后按任务名应用 config.taskModels 的模型路由覆盖
 * @param {Request} request - Next.js 请求对象
 * @param {string} [task] - 任务名（parse/explain/concepts/flashcards/analysis/action）
 * @returns {Promise<{config: Object, body: Object}>} 最终配置与请求体
 */
export async function resolveLLMConfig(request, task = '') {
  ...
  return { config: resolveTaskConfig(config, task), body };
}

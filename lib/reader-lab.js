...
import { READER_DEMO_GLOSSARY } from './reader-analysis.js';
...
// 填空翻转的稳定 key：source+target 组合唯一标识一条映射，
// 供派生文档回写原术语与装饰点击翻转共用
export function clozeMappingKey(mapping) {
...
export function applyReaderLabReplacements(source, mappings, view = 'source', sourceStart = 0, revealedKeys = null) {
  ...
  const replacements = (Array.isArray(mappings) ? mappings : [])
    .map((mapping, index) => {
      ...
      // 填空翻转：已翻开的映射在白话视图里回写『原术语』而非白话文本
      const revealed = Boolean(revealedKeys) && revealedKeys.has(clozeMappingKey(mapping));
      const text = view === 'target' && !revealed ? mapping?.target : mapping?.source;
...
/**
 * 批量分析记录的锚点身份：同一文档里同一位置、同一层级/标记类型的锚点
 * 只应存在一条记录。重点批量与解读批量来自同一次分析，锚点完全重合，
 * 用该键可识别跨批量的重复建档。
 */
export function batchAnchorKey(record) {
  return [
    record?.level || 'sentence',
    record?.markKind || '',
    Number.isInteger(record?.sourceStart) ? record.sourceStart : '',
    Number.isInteger(record?.sourceEnd) ? record.sourceEnd : '',
  ].join(':');
}

/**
 * 清理同一锚点上的重复批量记录：优先保留携带行间解读的一条，
 * 其余视为历史重复数据移除。返回 { records, removed }，removed 需由调用方从存储删除。
 */
export function dedupeBatchAnalysisRecords(records) {
  const list = Array.isArray(records) ? records : [];
  const groups = new Map();
  for (const record of list) {
    if (!record?.batchAnalysis || !Number.isInteger(record?.sourceStart)) continue;
  ...
  for (const group of groups.values()) {
    ...
    const ranked = [...group].sort((left, right) => (
      Boolean(right.explanation) - Boolean(left.explanation)
      || (right.updatedAt || 0) - (left.updatedAt || 0)
    ));
    for (const record of ranked.slice(1)) {
      removed.push(record);
      dropIds.add(record.id);
    }
  }
  if (removed.length === 0) return { records: list, removed };
  return { records: list.filter((record) => !dropIds.has(record.id)), removed };
}

// 旧版无 LLM Demo 曾在映射/解读里写入“（本地示例替换）”“本地 Demo 阅读辅助”占位文案，
// 已持久化的记录在恢复时用术语词典重写为真实值，避免占位文案继续上屏
const DEMO_PLACEHOLDER_MAPPING_TARGET = '本地示例替换';
const DEMO_PLACEHOLDER_DISPLAY = '本地 Demo 阅读辅助';

function findDemoGlossaryEntry(text) {
  if (!text) return null;
  // 长词优先，避免短词抢先命中（如“灰度”与“灰度发布”）
  const sorted = [...READER_DEMO_GLOSSARY].sort((a, b) => b.term.length - a.term.length);
  return sorted.find((entry) => text.includes(entry.term)) || null;
}

/** 修复旧 Demo 占位记录：映射换成词典真实值（无命中则丢弃），解读改为通俗首句 */
export function repairDemoPlaceholderRecords(records, { now = Date.now() } = {}) {
  const repaired = [];
  const next = (Array.isArray(records) ? records : []).map((record) => {
    if (!record?.batchAnalysis || !record.explanation) return record;
    const mappings = Array.isArray(record.explanation.mappings) ? record.explanation.mappings : [];
    const hasPlaceholderMapping = mappings.some((mapping) => (
      typeof mapping?.target === 'string' && mapping.target.includes(DEMO_PLACEHOLDER_MAPPING_TARGET)
    ));
    const display = typeof record.explanation.display === 'string' ? record.explanation.display : '';
    const hasPlaceholderDisplay = display.includes(DEMO_PLACEHOLDER_DISPLAY);
    if (!hasPlaceholderMapping && !hasPlaceholderDisplay) return record;

    const entry = findDemoGlossaryEntry(compactText(record.source || record.selectedText));
    const nextMappings = mappings.flatMap((mapping) => {
      if (!(typeof mapping?.target === 'string' && mapping.target.includes(DEMO_PLACEHOLDER_MAPPING_TARGET))) return [mapping];
      if (!entry) return [];
      return [{ source: entry.term, target: entry.plain, note: entry.note }];
    });
    // 占位解读换成原文首句本身；先去掉句尾标点再统一补句号，避免出现“。。”双尾
    const nextDisplay = hasPlaceholderDisplay
      ? `${compactText(record.source || record.selectedText).replace(/[。．.!！?？;；]+$/u, '')}。`
      : display;
    const updated = {
      ...record,
      explanation: {
        ...
        display: nextDisplay,
        plainExplanation: hasPlaceholderDisplay ? nextDisplay : record.explanation.plainExplanation,
      },
      demoRepairedAt: now,
      updatedAt: now,
    };
...
/** 修复旧 Demo 占位术语：术语词命中词典时释义换为词典大白话，未命中原样保留 */
export function repairDemoPlaceholderTerms(terms, { now = Date.now() } = {}) {
  const repaired = [];
  const next = (Array.isArray(terms) ? terms : []).map((term) => {
    if (!term || typeof term.explanation !== 'string' || !term.explanation.includes(DEMO_PLACEHOLDER_MAPPING_TARGET)) return term;
    const entry = findDemoGlossaryEntry(term.term);
    if (!entry) return term;
    const updated = { ...term, explanation: entry.plain, note: entry.note || term.note, updatedAt: now };
    repaired.push(updated);
    return updated;
  });
  return { terms: next, repaired };
}
...
// 角色卡片背面文案：把层级化重点的角色翻译成可记忆的定义
const DEMO_ROLE_MEANINGS = Object.freeze({
  core: '中心论点：全文论证所服务的最终结论，全文至多一条',
  subthesis: '分论点：段落层面支撑中心论点的次级结论',
  evidence: '论据：用来支撑论点的事实、数据或权威陈述',
  countermeasure: '对策：针对问题提出的具体行动建议',
  case: '案例：支撑论点的实际事例',
  concept: '概念：对关键术语的定义或界定',
  conclusion: '结论：由前提推断出的结果判断',
  background: '背景：铺垫性信息，不承载核心论证',
});

/**
 * 无 LLM 配置时的本地闪卡：直接从分析记录提取真实内容，
 * 角色卡问“这段重点属于哪一层”，术语卡问“大白话怎么理解”，不产出占位文案
 * @param {Array} records - 当前文档的分析记录
 * @param {string} title - 文档标题，用于卡片来源标注
 * @returns {Array<{front: string, back: string, source: string}>}
 */
export function createDemoFlashcards(records, title = '') {
  const list = Array.isArray(records) ? records : [];
  const cards = [];
  const seenFronts = new Set();
  const pushCard = (front, back, source) => {
    if (!front || !back || seenFronts.has(front)) return;
    seenFronts.add(front);
    cards.push({ front, back, source: source || title });
  };

  // 角色卡：取句子层重点（词语层标记不适合做问答卡），最多 6 张
  for (const record of list.filter((item) => item && item.level !== 'word').slice(0, 6)) {
    const meaning = DEMO_ROLE_MEANINGS[record.role];
    const quote = compactText(record.selectedText || record.source).slice(0, 40);
    if (!meaning || !quote) continue;
    pushCard(
      `《${title}》中「${quote}${(record.selectedText || '').length > 40 ? '…' : ''}」属于哪一层重点？`,
      meaning,
      quote
    );
  }

  // 术语卡：取精准替代映射的真实术语→大白话，最多 4 张
  const seenTerms = new Set();
  for (const record of list) {
    if (cards.length >= 10) break;
    for (const mapping of record?.explanation?.mappings || []) {
      const term = compactText(mapping?.source);
      const plain = compactText(mapping?.target);
      if (!term || !plain || seenTerms.has(term.toLowerCase())) continue;
      seenTerms.add(term.toLowerCase());
      pushCard(`「${term}」用大白话怎么理解？`, plain, term);
      if (cards.length >= 10) break;
    }
  }
...
// 内置 Demo 术语表：无 LLM 配置时知识面板也能看到背景定义效果
const DEMO_GLOSSARY_ENTRIES = Object.freeze([
  { term: 'RAG', aliases: ['检索增强生成'], explanation: '先检索资料再生成回答的 AI 架构，上线前需验收检索与生成两阶段。' },
  { term: 'Recall@5', aliases: [], explanation: '前 5 条检索结果中是否包含标准证据的比率，衡量检索阶段是否找对了资料。' },
  { term: '忠实度', aliases: ['faithfulness'], explanation: '生成回答是否严格依据检索到的证据，不编造、不偏离。' },
  { term: '幂等键', aliases: [], explanation: '同一次操作的去重凭证，绑定业务意图，防止重复提交产生第二个结果。' },
  { term: '间隔重复', aliases: ['spaced repetition'], explanation: '按遗忘规律安排复习时间，用更少的总时间记住更多内容。' },
]);

export function createDemoGlossary({ now = Date.now() } = {}) {
  return DEMO_GLOSSARY_ENTRIES.map((entry, index) => ({
    id: `demo-glossary-${index}`,
    term: entry.term,
...
// 目录从 Markdown 源文提取 ATX 标题：跳过围栏代码块避免误识别，去掉行尾闭合 # 与行内强调符号
export function extractMarkdownOutline(markdown) {
  ...
  for (const line of markdown.split(/\r?\n/)) {
    ...
    if (fenceMatch) {
      // 同类型围栏标记才视为闭合；嵌套或不同标记的代码行不影响外层围栏状态
    ...
    const text = headingMatch[2].replace(/[*_~`]/gu, '').replace(/\s+/gu, ' ').trim();
    if (!text) continue;
    outline.push({ level: headingMatch[1].length, text });
  }
  return outline;
}

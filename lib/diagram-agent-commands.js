...
import { createDiagramOperationMetrics, mergeDiagramOperationMetrics } from './diagram-operation-metrics.js';
import {
...
import { buildStreamTimeline, createDefaultMermaidPresentation, createDefaultPresentation, isDefaultMermaidPresentation, isDefaultPresentation, stripPseudoElements, timelineToPresentation } from './diagram-stream.js';
...
import { projectDiagram, projectDiagramAgentResult } from './diagram-agent-results.js';
import { getDiagramAgentBuildInfo } from './diagram-agent-protocol.js';
...
function scenePreflightError(preflight) {
  const error = new Error('Diagram scene failed the structural preflight.');
  error.code = 'SCENE_PREFLIGHT_FAILED';
...
function commitElementScene(drawing, scene, args, reason, now) {
  return commitDiagramScene(drawing, scene, {
    ...
    operationFingerprint: args.operationId ? operationFingerprint('element', args) : undefined,
    now,
  });
}
...
function idempotentReplay(drawing, args, tool) {
  ...
  const fingerprint = operationFingerprint(tool, args);
  if (previous.operationFingerprint && previous.operationFingerprint !== fingerprint) {
  ...
  return {
    ...
    scene: getDrawingScene(drawing),
  };
}

async function latestDrawing(repository, drawing) {
  if (typeof repository.drawings.get === 'function') {
    const direct = await repository.drawings.get(drawing.id);
    if (direct) return direct;
  }
  const drawings = await repository.drawings.list();
  return getDrawing(drawings, drawing.id);
}

async function commitPatchWithRetry({ repository, drawing, args, tool, apply, reason, now, metrics }) {
  const replay = idempotentReplay(drawing, args, tool);
  if (replay) return { replay, drawing };
  const maxRetries = Number.isInteger(args.maxConflictRetries)
    ? Math.max(0, Math.min(5, args.maxConflictRetries))
    : 2;
  const canRetry = args.retryOnConflict === true || args.expectedRevision === undefined || args.expectedRevision === null;
  let current = drawing;
  let attempt = 0;
  while (true) {
    const applied = apply(getDrawingScene(current), current);
    if (applied?.replay) return { replay: applied.replay, drawing: current, retries: attempt };
    const scene = applied?.scene || applied;
    try {
      const nextDrawing = commitDiagramScene(current, scene, {
        expectedRevision: attempt === 0 ? args.expectedRevision : getDiagramRevision(current),
        author: args.author || 'agent',
        reason: args.reason || reason,
        operationId: args.operationId,
        operationFingerprint: args.operationId ? operationFingerprint(tool, args) : undefined,
        now,
      });
      await repository.drawings.save(nextDrawing, { expectedRevision: getDiagramRevision(current) });
      return { drawing: nextDrawing, replay: null, retries: attempt };
    } catch (error) {
      if (error?.code !== 'REVISION_CONFLICT' || !canRetry) throw error;
      if (attempt >= maxRetries) {
        error.retryExhausted = true;
        error.conflictRetries = attempt;
        error.maxConflictRetries = maxRetries;
        throw error;
      }
      attempt += 1;
      metrics?.recordConflictRetry();
      current = await latestDrawing(repository, current);
    }
  }
}
...
function requestedElementMatches(existing, requested) {
  if (!existing || existing.isDeleted) return false;
  return Object.entries(requested).every(([key, value]) => (
    JSON.stringify(stableOperationValue(existing[key])) === JSON.stringify(stableOperationValue(value))
  ));
}

function instrumentRepository(repository, metrics) {
  if (!repository?.drawings || !metrics) return repository;
  const drawings = repository.drawings;
  return {
    ...repository,
    drawings: {
      ...drawings,
      list: (...args) => metrics.measure('repositoryReadMs', () => drawings.list(...args)),
      get: typeof drawings.get === 'function' ? (...args) => metrics.measure('repositoryReadMs', () => drawings.get(...args)) : undefined,
      save: (...args) => metrics.measure('repositoryWriteMs', () => drawings.save(...args)),
    },
  };
}

export function makeDrawing(args, { now = Date.now(), existingDrawings = [] } = {}) {
...
async function executeDiagramAgentCommandImpl(command, {
  repository,
  onOpen,
  onPresentation,
  screenshot,
  browserVersion = getDiagramAgentBuildInfo(),
  ...
  if (tool === 'read_diagram_guide') return getDiagramDesignGuide();
  ...
  switch (tool) {
    ...
    case 'verify_browser_connection':
      return {
        writable: true,
        browserVersion,
        diagramCount: drawings.length,
      };
    case 'get_diagram': {
      const drawing = getDrawing(drawings, args.id);
      return compactResponse ? projectDiagram(drawing, args) : { ...drawing, scene: getDrawingScene(drawing) };
    }
    ...
    case 'preflight_scene': {
      const scene = args.scene !== undefined
        ? normalizeSceneInput(args.scene)
        : getDrawingScene(getDrawing(drawings, args.id));
      return preflightDiagramScene(scene);
    }
    case 'verify_diagram': {
      const drawing = getDrawing(drawings, args.id);
      ...
      const description = isExcalidraw
        ? describeScene(scene)
        : `Mermaid diagram source is present (${String(drawing.source || '').length} characters).`;
      const visualRequested = args.visual !== false;
      let screenshotResult = null;
      let screenshotContent = null;
      if (visualRequested) {
        if (!isExcalidraw) {
          screenshotResult = { available: false, nextAction: 'verify_mermaid_render_in_workspace' };
        } else if (!screenshot) {
          screenshotResult = { available: false, nextAction: 'open_diagram_workspace_then_retry' };
        } else {
          const captured = await screenshot(drawing);
          screenshotResult = {
            available: true,
            ...(Array.isArray(captured?.content) ? { contentCount: captured.content.length } : {}),
          };
          screenshotContent = captured?.content;
        }
      }
      const nextAction = !preflight.ok
        ? 'fix_scene_then_verify'
        : (preflight.warnings.length > 0
          ? 'repair_scene_then_verify'
          : (visualRequested && !screenshotResult?.available ? screenshotResult?.nextAction || 'verify_visual_then_finish' : 'none'));
      const structuredContent = {
        id: drawing.id,
        routeId: drawing.routeId,
        revision: getDiagramRevision(drawing),
        description,
        preflight,
      ...
      return {
        ...(screenshotContent ? { content: screenshotContent } : {}),
        structuredContent,
      };
    }
    case 'query_diagram':
    ...
    case 'create_diagram': {
      const createArgs = tool === 'create_from_mermaid'
        ? { ...args, engine: 'mermaid', source: args.mermaidSyntax ?? args.mermaidDiagram ?? args.source }
        : args;
      if (tool === 'create_from_mermaid' && !String(createArgs.source || '').trim()) {
        throw new TypeError('create_from_mermaid requires mermaidSyntax, mermaidDiagram, or source');
      }
      const drawing = makeDrawing(createArgs, { now, existingDrawings: drawings });
      const preflight = drawing.engine === 'excalidraw' && drawing.scene
        ? preflightDiagramScene(drawing.scene)
        : null;
      if (preflight && !preflight.ok) throw scenePreflightError(preflight);
      await repository.drawings.save(drawing, { expectedRevision: 0 });
      const openRequested = createArgs.open !== false;
      ...
      const streamAutoPlayed = createArgs.stream === true && presentationAutoPlayed;
      ...
      return {
        ...
        ...(preflight ? { preflight, qualityWarnings: preflight.warnings } : {}),
        nextAction: preflight?.warnings?.length > 0 ? 'repair_scene_then_verify' : 'verify_diagram',
        ...(createArgs.stream === true ? { stream: true, streamAutoPlayed } : {}),
      };
    }
    case 'create_element': {
      const drawing = getDrawing(drawings, args.id);
      ...
      await repository.drawings.save(nextDrawing, { expectedRevision: getDiagramRevision(drawing) });
    ...
    case 'batch_create_elements': {
      ...
      const result = await commitPatchWithRetry({
        repository,
        drawing,
        args,
        tool,
        reason: 'batch-create-elements',
        now,
        metrics: operationMetrics,
        apply: (currentScene, currentDrawing) => {
          const existingById = new Map(currentScene.elements.map((element) => [String(element.id), element]));
          const existing = elements.filter((element) => existingById.has(String(element.id)));
          const collisions = existing
            .filter((element) => !requestedElementMatches(existingById.get(String(element.id)), element))
            .map((element) => String(element.id));
          if (collisions.length > 0) {
            const error = new Error(`Element IDs already exist: ${collisions.join(', ')}`);
            error.code = 'ELEMENT_ID_COLLISION';
            error.elementIds = collisions;
            throw error;
          }
          const missing = elements.filter((element) => !existingById.has(String(element.id)));
          if (missing.length === 0) {
            return {
              replay: {
                id: currentDrawing.id,
                routeId: currentDrawing.routeId,
                revision: getDiagramRevision(currentDrawing),
                stableIdReplay: true,
                elements,
                scene: currentScene,
              },
          ...
          return applyScenePatch(currentScene, { create: missing });
        },
      });
      if (result.replay) return result.replay;
      const nextDrawing = result.drawing;
      onOpen?.(nextDrawing, { open: false });
      return { id: nextDrawing.id, routeId: nextDrawing.routeId, revision: nextDrawing.revision, operationId: args.operationId || null, elements: elements.map((element) => nextDrawing.scene.elements.find((item) => item.id === element.id)), scene: nextDrawing.scene, conflictRetries: result.retries || 0 };
    }
    case 'batch_update_elements': {
      ...
      const result = await commitPatchWithRetry({
        ...
        args,
        tool,
        reason: 'batch-update-elements',
        now,
        metrics: operationMetrics,
        apply: (scene) => applyScenePatch(scene, { update: updates }),
      });
      if (result.replay) return result.replay;
      const nextDrawing = result.drawing;
      onOpen?.(nextDrawing, { open: false });
      return { id: nextDrawing.id, routeId: nextDrawing.routeId, revision: nextDrawing.revision, elements: updates.map(({ id }) => nextDrawing.scene.elements.find((item) => item.id === id)), scene: nextDrawing.scene, conflictRetries: result.retries || 0 };
    }
    case 'update_element': {
      const drawing = getDrawing(drawings, args.id);
      const scene = getDrawingScene(drawing);
    ...
    case 'align_elements': {
      const drawing = getDrawing(drawings, args.id);
      const scene = alignScene(getDrawingScene(drawing), {
        ids: args.elementIds || args.ids,
        alignment: args.alignment,
      });
      const nextDrawing = commitDiagramScene(drawing, scene, {
        ...args,
        author: args.author || 'agent',
        reason: 'align-elements',
        now,
      });
      await repository.drawings.save(nextDrawing, { expectedRevision: getDiagramRevision(drawing) });
      onOpen?.(nextDrawing, { open: false });
      return {
        id: nextDrawing.id,
        routeId: nextDrawing.routeId,
        revision: nextDrawing.revision,
        elementIds: args.elementIds || args.ids,
        alignment: args.alignment,
        scene: nextDrawing.scene,
        nextAction: 'describe_scene_then_get_canvas_screenshot',
      };
    }
    case 'distribute_elements': {
      const drawing = getDrawing(drawings, args.id);
      const scene = distributeScene(getDrawingScene(drawing), {
        ids: args.elementIds || args.ids,
    ...
    case 'get_canvas_screenshot': {
      const drawing = getDrawing(drawings, args.id);
      return screenshot ? screenshot(drawing) : browserScreenshot();
    }
    case 'share_diagram': {
      const drawing = getDrawing(drawings, args.id);
    ...
    case 'apply_diagram_patch': {
      ...
      const result = await commitPatchWithRetry({
        ...
        now,
        metrics: operationMetrics,
        apply: (scene) => applyRequestedScenePatch(scene, args.patch || {}),
      });
      if (result.replay) return result.replay;
      const nextDrawing = result.drawing;
      onOpen?.(nextDrawing, { open: false });
      return { id: nextDrawing.id, routeId: nextDrawing.routeId, revision: nextDrawing.revision, scene: nextDrawing.scene, conflictRetries: result.retries || 0 };
    }
...
export async function executeDiagramAgentCommand(command, options = {}) {
  const metrics = createDiagramOperationMetrics({
    operation: command?.tool || 'unknown',
    transport: options.transport || 'browser',
    payload: command,
  });
  const repository = instrumentRepository(options.repository, metrics);
  try {
    const result = await metrics.measure('browserExecutionMs', () => executeDiagramAgentCommandImpl(command, {
      ...options,
      repository,
      operationMetrics: metrics,
    }));
    const projectedResult = options.compactResponse ? projectDiagramAgentResult(command, result) : result;
    const operationMetrics = metrics.finish(projectedResult, 'ok');
    options.onMetrics?.(operationMetrics);
    return options.includeMetrics ? mergeDiagramOperationMetrics(projectedResult, operationMetrics) : projectedResult;
  } catch (error) {

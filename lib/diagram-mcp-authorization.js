...
const CODEX_MCP_SERVER_NAME = 'anchor-read-diagram';
...
function requestOrigin(requestOrUrl) {
  requestUrl(requestOrUrl);
  return resolveAnchorReadPublicOrigin({ baseUrl: requestOrUrl });
}
...
export function getDiagramMcpCodexOAuthSetup(requestOrUrl) {
  const endpoint = getDiagramMcpResourceUrl(requestOrUrl);
  return [
    `codex mcp add ${CODEX_MCP_SERVER_NAME} --url ${endpoint}`,
    `codex mcp login ${CODEX_MCP_SERVER_NAME}`,
  ].join('\n');
}

export function getDiagramMcpCodexPersonalTokenConfig(requestOrUrl, token) {
  const endpoint = getDiagramMcpResourceUrl(requestOrUrl);
  const bearerToken = String(token || '').trim();
  if (!bearerToken) throw new TypeError('A Personal Token is required to build the Codex MCP configuration.');
  return [
    ...
    `Authorization = ${JSON.stringify(`Bearer ${bearerToken}`)}`,
  ].join('\n');
}
 ...
 *
 * The OAuth endpoints use authorization-code + PKCE. A browser-issued
 * Personal Token is also available as a deliberately simple single-user mode.
 */
export function getDiagramMcpAuthorizationInfo(requestOrUrl, runtime = {}) {
  ...
  return {
    ...
    runtime: {
      ...
      requestBroker: runtime.requestBroker || 'memory',
      sharedRequestBroker: runtime.sharedRequestBroker === true,
      requestRoutingMultiInstance: runtime.requestRoutingMultiInstance === true,
      mcpSessionAffinityRequired: runtime.mcpSessionAffinityRequired !== false,

 ...
 *
 * AnchorRead uses OAuth for remote MCP clients and keeps browser pairing as a
 * separate workspace-binding layer. Legacy Bearer-token metadata remains in
 * the response so older clients can still connect.
 */
 ...
 *
 * The OAuth endpoints use authorization-code + PKCE. The legacy browser
 * pairing fields remain in this response so older clients can still offer a
 * manual Bearer-token setup when they do not implement OAuth.
 */
export function getDiagramMcpAuthorizationInfo(requestOrUrl, runtime = {}) {
  ...
  return {
    ...
    clientAction: {
      type: 'open_url',
      // This compatibility action has no client_id/code_challenge query, so
      // point legacy clients at the explanatory handoff page. OAuth-aware
      // clients must construct the authorization URL from server metadata.

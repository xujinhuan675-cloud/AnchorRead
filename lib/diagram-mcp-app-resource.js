...
<html lang="zh-CN">
  ...
  <body>
    ...
    <script type="module">
      ...
      import { ChevronLeft, ChevronRight, Pause, Play, Square } from 'https://esm.sh/lucide-react@1.31.0?deps=react@19.2.0';
      ...
      function AnchorReadDiagramAppCore({ app, handlersRef, pendingEventsRef }) {
        ...
        useEffect(() => {
          ...
          if (displayedElements.length && typeof api.scrollToContent === 'function') {
            window.requestAnimationFrame(() => {
              if (apiRef.current !== api) return;
              api.scrollToContent(displayedElements, {
                fitToContent: true,
                animate: presentationActive,
                duration: presentationActive ? 220 : 0,
              });
            });
          }
        ...
        return React.createElement('div', { className: 'app-shell' },
          ...
          presentationSteps.length
            ? React.createElement('div', { className: 'app-playback', role: 'group', 'aria-label': '图解播放' },
              ...
              React.createElement('button', { type: 'button', disabled: presentationIndex <= 0, 'aria-label': '上一步', title: '上一步', onClick: () => { setPresentationActive(true); setPresentationPlaying(false); setPresentationIndex((index) => Math.max(0, index - 1)); } }, React.createElement(ChevronLeft, { size: 16, strokeWidth: 2.2, 'aria-hidden': true })),
              React.createElement('span', { 'aria-live': 'polite' }, (presentationIndex + 1) + '/' + presentationSteps.length),
              React.createElement('button', { type: 'button', disabled: presentationIndex >= presentationSteps.length - 1, 'aria-label': '下一步', title: '下一步', onClick: () => { setPresentationActive(true); setPresentationPlaying(false); setPresentationIndex((index) => Math.min(presentationSteps.length - 1, index + 1)); } }, React.createElement(ChevronRight, { size: 16, strokeWidth: 2.2, 'aria-hidden': true })),
              presentationActive && React.createElement('button', { type: 'button', 'aria-label': '停止', title: '停止', onClick: () => { setPresentationActive(false); setPresentationPlaying(false); setPresentationIndex(0); } }, React.createElement(Square, { size: 15, strokeWidth: 2.2, 'aria-hidden': true })),
            )
            : null,
        );
      }

...
<html lang="zh-CN">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .app-playback-speed { width: 3.5rem; min-width: 3.5rem; border: 0; background: transparent; color: #57534e; font: .75rem ui-monospace, SFMono-Regular, Menlo, monospace; }
      .app-playback-select:focus-visible, .app-playback-speed:focus-visible { outline: 2px solid #a8a29e; outline-offset: 1px; }
  ...
  <body>
    ...
    <script type="module">
      ...
      import { Excalidraw, exportToSvg, convertToExcalidrawElements, restoreElements, FONT_FAMILY } from 'https://esm.sh/@excalidraw/excalidraw@0.18.0?deps=react@19.2.0,react-dom@19.2.0';
      ...
      const DEFAULT_STEP_DURATION_MS = ${DEFAULT_PRESENTATION_STEP_DURATION_MS};
      const DEFAULT_TRANSITION_MS = ${DEFAULT_PRESENTATION_TRANSITION_MS};
      const DEFAULT_REVEAL_MS = ${DEFAULT_PRESENTATION_REVEAL_MS};
      const DEFAULT_HOLD_MS = ${DEFAULT_PRESENTATION_HOLD_MS};
      const PLAYBACK_RATES = ${JSON.stringify(PRESENTATION_PLAYBACK_RATES)};
        ...
        try {
          ...
          const converted = convertToExcalidrawElements(withDefaults, { regenerateIds: false })
            .map((element) => element.type === 'text'
              ...
              : element);
          // The converter preserves stale text dimensions. Refresh them in
          // the embedded MCP App too, so an inline view matches the browser
          // canvas and thumbnail after a reload.
          const normalized = restoreElements(converted, null, {
            refreshDimensions: true,
            repairBindings: true,
          });
          return { elements: normalized, camera, complete: !partial };
        } catch {
      ...
      function connectorEndpointId(element, edge) {
        return compactLabel(element?.[edge + 'Binding']?.elementId || element?.[edge]?.id || element?.[edge + 'ElementId']);
      }

      function connectorElement(element) {
        return element?.type === 'arrow' || element?.type === 'line';
      }

      function genericElementTitle(element) {
        if (element?.type === 'diamond') return '判断节点';
        if (element?.type === 'rectangle' || element?.type === 'ellipse') return '节点';
        if (element?.type === 'text') return '说明';
        if (element?.type === 'image') return '图片';
        if (element?.type === 'frame') return '分组';
        if (element?.type === 'freedraw') return '绘制';
        if (connectorElement(element)) return '建立连接';
        return '图解元素';
      }
      ...
      function defaultElementPresentation(elements) {
        ...
        for (const element of source) {
          const label = compactLabel(element?.label?.text || element?.text || element?.originalText);
          if (element?.id && label) labels.set(String(element.id), label);
          if (element?.type === 'text' && element?.containerId && label && !labels.has(String(element.containerId))) {
            labels.set(String(element.containerId), label);
          }
        }
        const relationshipEndpointIds = new Set();
        for (const element of revealable) {
          if (!connectorElement(element)) continue;
          const startId = connectorEndpointId(element, 'start');
          const endId = connectorEndpointId(element, 'end');
          if (startId && endId && elementsById.has(startId) && elementsById.has(endId)) {
            relationshipEndpointIds.add(startId);
            relationshipEndpointIds.add(endId);
          }
        }
        const visibleIds = [];
        ...
        const reveal = (element) => {
          ...
          visible.add(id);
          visibleIds.push(id);
          const ownLabel = labels.get(id) || '';
          let title = ownLabel || genericElementTitle(element);
          if (connectorElement(element)) {
            const startId = connectorEndpointId(element, 'start');
            const endId = connectorEndpointId(element, 'end');
            ...
            if (startLabel && endLabel) title = ownLabel ? startLabel + ' —' + ownLabel + '→ ' + endLabel : startLabel + ' → ' + endLabel;
          }
          frames.push({ currentId: id, title, visibleElementIds: [...visibleIds] });
        };
        for (const element of revealable) {
          if (!connectorElement(element) && relationshipEndpointIds.has(String(element.id))) continue;
          const startId = connectorElement(element) ? connectorEndpointId(element, 'start') : '';
          const endId = connectorElement(element) ? connectorEndpointId(element, 'end') : '';
          if (startId && endId && elementsById.has(startId) && elementsById.has(endId)) {
        ...
        const maxSteps = 100;
        const groupSize = Math.max(1, Math.ceil(frames.length / maxSteps));
        const limited = frames.length <= maxSteps
          ? frames
          : frames.filter((_frame, index) => (index + 1) % groupSize === 0 || index === frames.length - 1);
        return limited.map((frame, index) => ({
          id: 'step-' + (index + 1),
          title: frame.title,
      ...
      function normalizePresentation(value, fallback) {
        ...
        return {
          ...
          steps: value.steps.slice(0, 100).map((step, index) => ({
            ...
            durationMs: Number.isFinite(Number(step?.durationMs)) ? Math.max(0, Math.min(60000, Number(step.durationMs))) : DEFAULT_STEP_DURATION_MS,
            transitionMs: Number.isFinite(Number(step?.transitionMs)) ? Math.max(0, Math.min(10000, Number(step.transitionMs))) : DEFAULT_TRANSITION_MS,
            revealMs: Number.isFinite(Number(step?.revealMs)) ? Math.max(0, Math.min(10000, Number(step.revealMs))) : DEFAULT_REVEAL_MS,
            holdMs: Number.isFinite(Number(step?.holdMs)) ? Math.max(0, Math.min(60000, Number(step.holdMs))) : DEFAULT_HOLD_MS,
      ...
      function presentationTransitionDuration(step) {
        ...
        return reducedMotion ? 0 : Math.max(0, Math.min(1200, Number.isFinite(requested) ? requested : DEFAULT_TRANSITION_MS));
      }

      function presentationStepDuration(step, playbackRate) {
        const duration = Number.isFinite(Number(step?.durationMs)) ? Number(step.durationMs) : DEFAULT_STEP_DURATION_MS;
        const transition = Number.isFinite(Number(step?.transitionMs)) ? Number(step.transitionMs) : DEFAULT_TRANSITION_MS;
        const reveal = Number.isFinite(Number(step?.revealMs)) ? Number(step.revealMs) : DEFAULT_REVEAL_MS;
        const hold = Number.isFinite(Number(step?.holdMs)) ? Number(step.holdMs) : DEFAULT_HOLD_MS;
        const hasCameraMotion = Boolean(step?.camera || step?.focusElementIds?.length);
        const motion = Math.max(reveal, hasCameraMotion ? transition : 0);
        const rate = Math.max(0.5, Math.min(2, Number(playbackRate) || 1));
        return Math.round(Math.max(duration, motion + hold) / rate);
      }
      ...
      function elementsFitSafeViewport(elements, state) {
        if (!elements.length) return true;
        const zoom = Number(state?.zoom?.value) || 1;
        const viewportWidth = (Number(state?.width) || 0) / zoom;
        const viewportHeight = (Number(state?.height) || 0) / zoom;
        if (viewportWidth <= 0 || viewportHeight <= 0) return false;
        const inset = 0.12;
        const left = -(Number(state?.scrollX) || 0) + viewportWidth * inset;
        const top = -(Number(state?.scrollY) || 0) + viewportHeight * inset;
        const right = left + viewportWidth * (1 - inset * 2);
        const bottom = top + viewportHeight * (1 - inset * 2);
      ...
      function viewBoxContains(outer, inner, inset = 0.1) {
        if (!outer || !inner) return false;
        return inner.x >= outer.x + outer.width * inset
          && inner.y >= outer.y + outer.height * inset
          && inner.x + inner.width <= outer.x + outer.width * (1 - inset)
          && inner.y + inner.height <= outer.y + outer.height * (1 - inset);
      }
      ...
      function AnchorReadDiagramAppCore({ app, handlersRef, pendingEventsRef }) {
        ...
        const [diagramResourceKind, setDiagramResourceKind] = useState('');
        ...
        const presentationSteps = useMemo(() => {
          ...
          const legacyGenerated = presentation?.title === '流式重放'
            && presentation?.steps?.every((step, index) => step?.id === 'stream-' + (index + 1)
              && Number(step?.durationMs) <= 500
              && step?.focusElementIds?.length);
          return legacyGenerated ? fallback : normalizePresentation(presentation, { title: '', steps: fallback })?.steps || fallback;
        }, [engine, elements, presentation, source]);
        const presentationHasNamedSteps = presentationSteps.some((step) => Boolean(step.title));
        ...
        useEffect(() => {
          ...
          }, presentationStepDuration(presentationStep, playbackRate));
          return () => window.clearTimeout(timer);
        }, [playbackRate, presentationIndex, presentationPlaying, presentationStep, presentationSteps]);
        ...
        useEffect(() => {
          const applyInput = (input, partial = false) => {
            ...
            if (args.openResource?.kind) setDiagramResourceKind(String(args.openResource.kind));
          };
          const applyResult = (result) => {
            // Prefer the typed result channel when a host does not forward
            // ontoolinput. Never parse arbitrary result text: it can mistake
            // resource HTML or a human-readable note for Mermaid source and
            // overwrite an already-rendered canvas. Some MCP host bridges
            // serialize the field with Rust's snake_case naming, so accept
            // both spellings across hosts.
            ...
            if (value && typeof value === 'object') {
              ...
              if (value.openResource?.kind) setDiagramResourceKind(String(value.openResource.kind));
        ...
        // MCP hosts can attach and resize the iframe after the tool payload has
        // already hydrated the app. Excalidraw measures its container at mount;
        // refresh it again when the host gives the canvas its real size so the
        // first render does not depend on a later collapse/source interaction.
        useEffect(() => {
          if (!api || !canvasHostRef.current || !elements.length) return undefined;
          let frame = 0;
          const refreshCanvas = () => {
            ...
            frame = requestAnimationFrame(() => {
              ...
              if (presentationStep?.camera || typeof api.scrollToContent !== 'function') return;
          ...
          refreshCanvas();
          const observer = typeof ResizeObserver === 'function' ? new ResizeObserver(refreshCanvas) : null;
          observer?.observe(canvasHostRef.current);
          return () => {
            cancelAnimationFrame(frame);
            observer?.disconnect();
          };
        }, [api, displayedElements, elements.length, presentationActive]);

        useEffect(() => {
          if (!api || !elements.length || typeof api.updateScene !== 'function' || displayedElementsKey === lastAppliedElements) return;
          const nextVisibleIds = new Set(displayedElements.filter((element) => Number(element?.opacity) !== 0).map((element) => element.id));
          const newlyVisibleIds = new Set([...nextVisibleIds].filter((id) => !presentationVisibleIdsRef.current.has(id)));
          presentationVisibleIdsRef.current = presentationActive ? nextVisibleIds : new Set();
          const reducedMotion = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches;
          const revealDuration = reducedMotion
            ? 0
            : Math.max(0, Math.min(1200, Number(presentationStep?.revealMs) || DEFAULT_REVEAL_MS));
          cancelAnimationFrame(revealAnimFrameRef.current);
          if (presentationActive && revealDuration > 0 && newlyVisibleIds.size > 0) {
            const revealStart = displayedElements.map((element) => newlyVisibleIds.has(element.id) ? { ...element, opacity: 0 } : element);
            api.updateScene({ elements: revealStart, appState, files });
            const startedAt = performance.now();
            const tick = (now) => {
              const progress = Math.min(1, (now - startedAt) / revealDuration);
              const eased = easeInOutQuad(progress);
              api.updateScene({ elements: displayedElements.map((element) => newlyVisibleIds.has(element.id)
                ? { ...element, opacity: Math.round((Number(element.opacity) || 0) * eased) }
                : element) });
          ...
          return () => cancelAnimationFrame(revealAnimFrameRef.current);
        }, [api, appState, displayedElements, displayedElementsKey, elements, files, lastAppliedElements, presentationActive, presentationStep]);
        ...
        useEffect(() => {
          ...
          const focusElements = displayedElements.filter((element) => focusIds.has(element?.id) || (element?.containerId && focusIds.has(element.containerId)));
          const readableFocusElements = focusElements.filter((element) => element?.type !== 'arrow' && element?.type !== 'line');
          ...
          if (presentationStep.camera && typeof api.updateScene === 'function') {
            ...
            if (!cameraTargetChanged(from, target)) return undefined;
            const startedAt = performance.now();
            cancelAnimationFrame(presentationAnimFrameRef.current);
            const tick = (now) => {
          ...
          } else if (typeof api.scrollToContent === 'function' && !presentationViewportReadyRef.current) {
            presentationViewportReadyRef.current = true;
            lastFocusCameraAtRef.current = performance.now();
            api.scrollToContent(displayedElements, { fitToContent: true, animate: true, duration });
          } else if (readableFocusElements.length && typeof api.scrollToContent === 'function') {
            const now = performance.now();
            if (!elementsFitSafeViewport(readableFocusElements, currentState)
              && now - lastFocusCameraAtRef.current >= FOCUS_CAMERA_COOLDOWN_MS) {
              lastFocusCameraAtRef.current = now;
              api.scrollToContent(readableFocusElements, { fitToContent: true, animate: true, duration });
            }
          }
        ...
        useEffect(() => {
          ...
          const target = presentationActive && presentationStep?.camera
            ...
            : presentationActive && focusViewBox && !viewBoxContains(from, focusViewBox)
              ? focusViewBox
              : presentationActive ? from : base;
          if (!target || !base || sameViewBox(from, target)) return undefined;
          const duration = presentationTransitionDuration(presentationStep);
          const startedAt = performance.now();
          cancelAnimationFrame(presentationAnimFrameRef.current);
          const tick = (now) => {
        ...
        const openButtonLabel = diagramResourceKind === 'diagram'
          ? '在 AnchorRead 中打开当前图解'
          : '打开 AnchorRead 图解工作区';
        ...
        return React.createElement('div', { className: 'app-shell app-display-' + displayMode },
          React.createElement('div', { className: 'app-toolbar' },
            ...
            React.createElement('div', { className: 'app-toolbar-actions' },
              hostSupportsOpenLinks && diagramUrl
                ? React.createElement('button', { className: 'app-button', type: 'button', onClick: openAnchorRead }, openButtonLabel)
                : null,
          ...
          React.createElement('div', { ref: canvasHostRef, className: 'app-canvas' + (editing ? ' app-editing' : ' app-focus-mode') },
          ...
          presentationSteps.length
            ? React.createElement('div', { className: 'app-playback ' + (playbackExpanded ? 'app-playback-expanded' : 'app-playback-collapsed'), role: 'group', 'aria-label': '图解播放' },
              ...
              presentationHasNamedSteps && React.createElement('select', { className: 'app-playback-secondary app-playback-select', value: presentationIndex, 'aria-label': '选择演示阶段', onChange: (event) => { setPresentationActive(true); setPresentationPlaying(false); setPresentationIndex(Number(event.target.value)); } }, presentationSteps.map((step, index) => React.createElement('option', { key: step.id || index, value: index }, step.title || '阶段 ' + (index + 1)))),
              React.createElement('select', { className: 'app-playback-secondary app-playback-speed', value: playbackRate, 'aria-label': '播放速度', title: '播放速度', onChange: (event) => setPlaybackRate(Number(event.target.value)) }, PLAYBACK_RATES.map((rate) => React.createElement('option', { key: rate, value: rate }, rate + '×'))),

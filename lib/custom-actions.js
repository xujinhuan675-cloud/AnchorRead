...
export function createCustomAction(input = {}, { now = Date.now(), generateId = defaultGenerateId } = {}) {
  ...
  return {
    ...
    // 工具栏排序依据：缺省用创建时间，兼容存量数据
    order: Number.isFinite(input.order) ? input.order : now,
...
// 内置 Demo 自定义动作：无 LLM 配置时仍可在选区 BubbleMenu 里看到效果
const DEMO_CUSTOM_ACTION_DEFS = Object.freeze([
  {
    id: 'demo-extract-key-point',
    name: '提炼要点',
    description: '提取选区的核心要点，用一句话概括',
    promptTemplate: '提取以下文本的核心要点，用一句话概括：{{selection}}',
  },
  {
    id: 'demo-challenge-question',
    name: '反问检验',
    description: '针对选区提出一个批判性问题',
    promptTemplate: '针对以下文本提出一个批判性问题，检验其逻辑是否成立：{{selection}}',
  },
  {
    id: 'demo-analogy',
    name: '类比联想',
    description: '用日常类比解释选区核心概念',
    promptTemplate: '用一个日常类比解释以下文本的核心概念：{{selection}}',
  },
]);

export function createDemoCustomActions({ now = Date.now() } = {}) {
  return DEMO_CUSTOM_ACTION_DEFS.map((def, index) => ({
    ...def,
    enabled: true,
    order: now + index,
    createdAt: now + index,
...
/** 无 LLM 配置时对选区执行 Demo 动作，返回确定性的本地回应 */
export function createDemoCustomActionResult(action, selection) {
  const text = typeof selection === 'string' ? selection.trim() : '';
  const firstSentence = (text.split(/[。！？\n]/u)[0] || '').slice(0, 60) || text.slice(0, 60);
  const name = action?.name || '自定义动作';
  if (name.includes('要点') || name.includes('提炼')) {
    return `核心要点：${firstSentence}。\n其余内容围绕此要点展开，理解了这句就抓住了主线。`;
  }
  if (name.includes('反问') || name.includes('检验')) {
    return `针对「${firstSentence}」的检验：\n这一判断依赖什么前提？如果前提不成立，结论是否还成立？是否有反面案例？`;
  }
  if (name.includes('类比') || name.includes('联想')) {
    return `可以类比为：${firstSentence}——就像日常生活中先确认依据再下结论，而不是凭感觉拍板。`;
  }
  return `${name}：${firstSentence}。\n（以上为本地 Demo 回应，配置 LLM 后可按动作提示词生成更细致的结果。）`;
}

...
import { getDiagramRouteId } from './diagram-route-id.js';
import { buildDiagramPath, buildNewDiagramPath } from './workspace-routes.js';
...
export function buildDiagramEditorHref(drawing) {
  return buildDiagramPath(getDiagramRouteId(drawing));
}

export function buildNewDiagramHref() {
  return buildNewDiagramPath();
}
...
export function duplicateDrawing(drawing, {
  ...
  const { routeId: _routeId, ...sourceDrawing } = drawing;
  return {
    ...sourceDrawing,
    id,

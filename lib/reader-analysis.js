...
export function mergeReaderAnalysisResponses(responses, request) {
  ...
  const anchors = [...anchorsByRange.values()]
    ...
    .slice(0, MAX_ANCHORS);
  const explanations = [...explanationsByBlock.values()]
    .sort((left, right) => left.sourceStart - right.sourceStart)
    .slice(0, MAX_EXPLANATIONS);
  if (anchors.length === 0 || explanations.length === 0) {
...
export function normalizeReaderAnalysisResponse(
  ...
  if ((!allowEmpty && result.anchors.length === 0) || result.anchors.length > MAX_ANCHORS) {
    throw new ReaderAnalysisResponseError('AI 返回的 anchors 数量无效');
  }
  if (
    (!allowEmpty && result.explanations.length === 0) ||

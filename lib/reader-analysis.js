...
function createDemoMapping(block) {
  ...
  const segment = (cut > 0 ? body.slice(0, cut) : body).slice(0, 16).trim();
  if (!segment) return [];
  return [{
    source: segment,
    ...
    note: '本地 Demo 示例替换',
    aliases: [],
    start: block.sourceStart + prefix.length,
    end: block.sourceStart + prefix.length + segment.length,
  }];
}
...
export function createDemoReaderAnalysis(request) {
  ...
  for (const block of spansByBlock.keys()) {
    ...
    // 候选词限制在单个分句内（不跨逗号/句号等分隔），滑窗枚举全部 2-8 字汉字串，
    // 优先 4 字词（成语/固定搭配）再及短词，过滤虚词开头，避免词标切断语义或选到铺垫
    const clauseBodies = body.split(/[，。；：、！？,.;:!?]/u);
    const wordCandidates = [];
    let clauseOffset = 0;
    for (const clause of clauseBodies) {
      for (const match of clause.matchAll(/(?=([\u4e00-\u9fff]{2,8}))/gu)) {
        wordCandidates.push({ text: match[1], offset: clauseOffset + match.index });
      }
      clauseOffset += clause.length + 1;
    }
    const wordRank = { 4: 0, 3: 1, 2: 2, 5: 3, 6: 4, 7: 5, 8: 6 };

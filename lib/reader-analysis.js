...
export function buildReaderAnalysisPrompt(input) {
  ...
  // 解释深度档位：控制 mapping 覆盖密度与 display/note 的详尽程度
  const DEPTH_CLAUSES = Object.freeze({
    light: '12. 解释深度为轻度：只为真正的生僻术语、缩写与专业表达生成 mapping，常见词汇与通顺表述一律不替换；display 保持一句话，note 留空；不含难点的原文块不生成 explanation。',
    standard: '12. 解释深度为标准：优先覆盖术语、缩写、生僻词与专业表达，display 用一两句自然说明，note 简短。',
    deep: '12. 解释深度为深度：除术语与缩写外，复杂句式、隐含前提与背景知识也要通过 display 拆解，可补充类比、举例或背景；mapping 覆盖更广，note 可说明替换理由；除纯结构性内容外尽量逐块给出 explanation。',
  });
...
export function createDemoReaderAnalysis(request) {
  ...
  // Demo 同样响应解释深度：轻度只覆盖少量块，深度尽量铺开，让档位切换在无 LLM 时也可见
  const DEPTH_DEMO_BLOCKS = Object.freeze({ light: 3, standard: 5, deep: 8 });
  const demoBlockLimit = DEPTH_DEMO_BLOCKS[source.depth] ?? DEPTH_DEMO_BLOCKS.standard;
  ...
  return {
    ...
    summary,
    anchors,
    explanations: candidates.slice(0, Math.min(demoBlockLimit, candidates.length)).map((block) => ({
      blockId: block.id,
      source: block.source,
      sourceStart: block.sourceStart,
      sourceEnd: block.sourceEnd,

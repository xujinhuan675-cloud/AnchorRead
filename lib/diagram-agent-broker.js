...
function selectActiveClient(state, request, now = Date.now()) {
  ...
  return clients.sort((left, right) => (
    Number(right.visible && right.focused) - Number(left.visible && left.focused)
...
function requestTargetsClient(request, client) {
  ...
  const targetBindingId = String(request.scope?.bindingId || '').trim();
  ...
  if (targetBindingId && targetBindingId !== client.bindingId) return false;
...
export function createDiagramAgentRequest(payload, {
  ...
  const request = {
    ...
    scope: scope ? {
      ...
      bindingId: String(scope.bindingId || '').trim(),
...
export function claimDiagramAgentRequests(clientId, { limit = 4, client = {} } = {}) {
  ...
  if (!diagramAgentBuildCompatibility(registered).compatible) return [];
  const claimed = [];
...
export function resolveDiagramAgentRequest(requestId, claimToken, result, error) {
  ...
  if (error) {
    if (error instanceof Error) request.reject(error);
      ...
      const restored = new Error(String(error?.message || error));
      Object.assign(restored, serializeDiagramAgentError(error));
      request.reject(restored);
    }
  }

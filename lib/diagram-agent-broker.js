...
function cleanup(state = getState(), now = Date.now()) {
  for (const [requestId, request] of state.requests) {
    if (request.expiresAt <= now && request.status !== 'resolved') {
      ...
      clearTimeout(request.expiryTimer);
...
function selectActiveClient(state, request, now = Date.now()) {
  const clients = [...state.clients.values()].filter((client) => (
    client.expiresAt > now && requestTargetsClient(request, client, request.wakeOnly ? request.id : '')
  ));
...
function requestTargetsClient(request, client, wakeRequestId = '') {
  if (request.wakeOnly && String(wakeRequestId || '').trim() !== request.id) return false;
...
export function createDiagramAgentRequest(payload, {
  ...
  request.expiryTimer = setTimeout(() => {
    cancelDiagramAgentRequest(requestId, new Error('Diagram bridge request expired before the browser responded.'));
  }, Math.max(1, request.expiresAt - Date.now()));
  request.expiryTimer.unref?.();
  return { id: requestId, promise };
}

export function claimDiagramAgentRequests(clientId, { limit = 4, client = {}, wakeRequestId = '' } = {}) {
  const state = getState();
  cleanup(state);
  const normalizedClientId = String(clientId || '').trim();
  ...
  while (remaining > 0 && state.pending.length && claimed.length < max) {
    ...
    if (!requestTargetsClient(request, registered, wakeRequestId)) {
      state.pending.push(requestId);
      continue;
    }
    // A wake-capability request deliberately targets the newly opened page,
    // even when another normal bridge tab is currently focused.
    const active = request.wakeOnly ? null : selectActiveClient(state, request);
...
export async function waitForDiagramAgentRequests(clientId, { limit = 4, waitMs = 20_000, client = {}, wakeRequestId = '' } = {}) {
  const deadline = Date.now() + Math.max(0, Math.min(Number(waitMs) || 0, 25_000));
  do {
    const requests = claimDiagramAgentRequests(clientId, { limit, client, wakeRequestId });

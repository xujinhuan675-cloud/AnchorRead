...
export class DiagramMcpOAuthStore {
  ...
  listAuthorizations({ workspaceId, browserSessionId, bindingId } = {}, now = Date.now()) {
    this.cleanup(now);
    const normalizedWorkspaceId = String(workspaceId || '').trim();
    const normalizedBrowserSessionId = String(browserSessionId || '').trim();
    const normalizedBindingId = String(bindingId || '').trim();
    if (!normalizedWorkspaceId || !normalizedBrowserSessionId || !normalizedBindingId) return [];
    return [...this.refreshTokens.values()]
      .filter((record) => (
        record.browserContext?.workspaceId === normalizedWorkspaceId
        && record.browserContext?.browserSessionId === normalizedBrowserSessionId
        && record.browserContext?.bindingId === normalizedBindingId
      ))
      .sort((left, right) => Number(right.createdAt || 0) - Number(left.createdAt || 0))
      .map((record) => publicAuthorization(record, now));
  }

  revokeAuthorizations({ workspaceId, browserSessionId, bindingId } = {}, { clientId = '', now = Date.now() } = {}) {
    this.cleanup(now);
    const normalizedWorkspaceId = String(workspaceId || '').trim();
    const normalizedBrowserSessionId = String(browserSessionId || '').trim();
    const normalizedBindingId = String(bindingId || '').trim();
    ...
    for (const [tokenHash, replay] of this.refreshTokenReplays) {
      if (replay.record?.browserContext?.workspaceId !== normalizedWorkspaceId
        || replay.record?.browserContext?.browserSessionId !== normalizedBrowserSessionId
        || replay.record?.browserContext?.bindingId !== normalizedBindingId

  ...
  load() {
    ...
    try {
      ...
      for (const client of data.clients || []) {
        if (client?.clientId && Array.isArray(client.redirectUris)) this.clients.set(client.clientId, client);
      }
      for (const transaction of data.transactions || []) {
        if (transaction?.id) this.transactions.set(transaction.id, transaction);
      }
      for (const code of data.codes || []) {
        if (code?.codeHash) this.codes.set(code.codeHash, code);
      }
      for (const refresh of data.refreshTokens || []) {
        if (refresh?.tokenHash) this.refreshTokens.set(refresh.tokenHash, refresh);
      }
    } catch {

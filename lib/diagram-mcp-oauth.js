...
const MAX_REFRESH_AUTHORIZATIONS_PER_GRANT = 8;
...
export class DiagramMcpOAuthStore {
  ...
  cleanup(now = Date.now()) {
    ...
    // cached client id. Explicit browser authorizations remain independent
    // across sessions, while legacy duplicates and excessive old grants are
    // compacted to a bounded recent set.
    const authorizationsByGrant = new Map();
    for (const [tokenHash, refresh] of this.refreshTokens) {
      ...
      const entries = authorizationsByGrant.get(grantKey) || [];
      entries.push({ tokenHash, refresh });
      authorizationsByGrant.set(grantKey, entries);
    }
    for (const entries of authorizationsByGrant.values()) {
      entries.sort((left, right) => Number(right.refresh.createdAt || 0) - Number(left.refresh.createdAt || 0));
      const hasExplicitGrant = entries.some(({ refresh }) => String(refresh?.grantId || '').trim());
      const keep = hasExplicitGrant ? MAX_REFRESH_AUTHORIZATIONS_PER_GRANT : 1;
      for (const { tokenHash } of entries.slice(keep)) {
  ...
  createRefreshToken(record, now = Date.now()) {
    ...
    this.refreshTokens.set(tokenHash, {
      ...
      scopes,
      // A fresh browser authorization gets its own grant. Rotation passes the
      // existing grant id through, so only the rotated token is replaced.
      grantId: String(record?.grantId || '').trim() || randomId('grant', 12),
      createdAt: now,

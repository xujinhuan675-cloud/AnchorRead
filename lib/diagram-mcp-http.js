...
import { getDiagramMcpAuthorizationInfoUrl } from './diagram-mcp-authorization.js';
import {
...
export async function authorizeMcpRequest(request) {
  ...
  if (!supplied) {
    return { ok: false, status: 401, code: 'UNAUTHORIZED', message: 'MCP authentication failed. Use a paired AnchorRead Bearer token.' };
  }
  try {
...
export function mcpCorsHeaders(request) {
  const headers = {
    ...
    'Access-Control-Allow-Headers': 'Authorization, Content-Type, MCP-Protocol-Version, MCP-Session-Id, Last-Event-ID, X-API-Key',
    Vary: 'Origin',
  };
  const origin = String(request.headers.get('origin') || '').trim();
...
function responseFor(request, body, { status = 200, sessionId: id, contentType = 'application/json' } = {}) {
  ...
  const headers = {
    ...
    'X-AnchorRead-Routing-Mode': runtime.persistentTokens ? 'single-process-persistent-pairing' : 'single-process-memory',
  };
  if (id) {
...
export async function handleDiagramMcpHttpRequest(request, {
  ...
  if (!auth.ok) {
    const authorizationHeader = auth.status === 401
      ? `Bearer resource_metadata="${getDiagramMcpAuthorizationInfoUrl(request)}"`
      : '';
  ...
  try {
    const result = await submitTool(name, rpc.params?.arguments || {}, {
      signal: request.signal,
      binding: auth.binding,
      tokenId: auth.token?.id || '',
    });
    return responseFor(request, { jsonrpc: '2.0', id: rpc.id ?? null, result: textResult(result) }, { sessionId: session.id });
  } catch (error) {

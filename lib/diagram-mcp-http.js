...
function textResult(value) {
  ...
  if (result.structuredContent === undefined && value && typeof value === 'object' && !Array.isArray(value)) {
...
export async function queueDiagramToolForBrowser(tool, args, {
  binding = null,
  tokenId = '',
  ttlMs = 2 * 60_000,
} = {}) {
  ...
  const { id, promise } = await transport.createRequest({ tool, args }, {
    ttlMs,
    // The wake request id is an unguessable, short-lived capability carried
    // only by the returned link. Do not bind it to the old browser profile:
    // the operating system's default browser may use a different profile.
    scope: null,
    tokenId,
    wakeOnly: true,
  });
  // The MCP call returns immediately so its resource link can wake the
  // browser. Resolution is completed by the browser after this response.
  promise.catch(() => {});
  return id;
}

async function hasBrowserTarget(binding = null) {
  if (binding) return binding.connected === true;
  const snapshot = await getDiagramAgentTransport().snapshot?.();
  if (!snapshot) return false;
  return Array.isArray(snapshot.clients) && snapshot.clients.some((client) => client.visible);
}
...
async function callDiagramTool(name, args, { request, auth, submitTool }) {
  ...
  if (name === 'create_diagram' && args.open !== false && submitTool === submitDiagramTool) {
    if (await hasBrowserTarget(auth.binding)) {
    ...
    const requestId = await queueDiagramToolForBrowser(name, args, {
      binding: auth.binding,
      tokenId: auth.token?.id || '',
    });
    return createDeferredDiagramResult(args, requestId, { baseUrl: request });
  }
  return submitTool(name, args, {
    signal: request.signal,
    binding: auth.binding,
  ...
  try {
    ...
    const result = await traceMcpTool(name, () => (
      name === 'verify_browser_connection'
        ? verifyDiagramBrowserConnection({ request, auth, submitTool })
    ...
    const inline = name === 'create_diagram' && browserUnavailable
      ? createInlineDiagramResult(rpc.params?.arguments || {}, error, { baseUrl: request })
      : null;
    if (inline) {

...
async function callDiagramTool(name, args, { request, auth, submitTool }) {
  ...
  if (name === 'create_diagram' && args.open !== false && submitTool === submitDiagramTool) {
    ...
    return createDeferredDiagramResult(args, requestId, { baseUrl: request.url });
  }
  return submitTool(name, args, {
    signal: request.signal,
    binding: auth.binding,
  ...
  if (name === 'open_diagram_workspace') {
    return responseFor(request, {
      ...
      result: textResult({
        ...
        openResource: { kind: 'workspace', url: buildDiagramWorkspaceUrl({ baseUrl: request.url }) },
      }),
    }, { sessionId: session.id });
  }
  try {
    const args = rpc.params?.arguments || {};
    const result = await traceMcpTool(name, () => callDiagramTool(name, args, { request, auth, submitTool }));
    return responseFor(request, {
      jsonrpc: '2.0',
      id: rpc.id ?? null,
    ...
    const inline = name === 'create_diagram' && browserUnavailable
      ? createInlineDiagramResult(rpc.params?.arguments || {}, error, { baseUrl: request.url })
      : null;
    if (inline) {

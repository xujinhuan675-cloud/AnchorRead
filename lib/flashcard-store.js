...
import { createCard, reviewCard } from './fsrs.js';
...
export const flashcardStore = {
   ...
   */
  addCards(items, articleTitle = '', documentId = '') {
    const existing = loadCards();
    // 去重：问题与来源文章均相同的卡片不再重复添加
    const existingKeys = new Set(
      existing.map((c) => `${c.documentId || c.articleTitle}||${c.front}`)
    );

    const created = [];
    for (const item of items || []) {
      if (!item || !item.front) continue;
      const key = `${documentId || articleTitle}||${item.front}`;
  ...
  getDueCards(now = Date.now(), documentId = '') {
    return loadCards()
      .filter((c) => c.due <= now && (!documentId || c.documentId === documentId))
      .sort((a, b) => a.due - b.due);
  },

  /** 获取到期卡片数量 */
  getDueCount(now = Date.now(), documentId = '') {
    return this.getDueCards(now, documentId).length;
  },
  ...
  removeForDocument(documentId) {
    ...
    saveCards(loadCards().filter((card) => card.documentId !== documentId));
  },
  ...
  getForDocument(documentId) {
    return loadCards().filter((card) => card.documentId === documentId);
  },

  /** Replace local cards when restoring an Anchor Read workspace file. */
  replaceAll(cards) {
    saveCards(Array.isArray(cards) ? cards : []);
  },

  /** 统计信息：总数与今日已复习数 */
  getStats(now = Date.now(), documentId = '') {
    const cards = documentId ? this.getForDocument(documentId) : loadCards();

 ...
 * 仅作为不支持 File System Access API 的浏览器回退方案
 */
...
/**
 * 把笔记集合直接写入用户选择的文件夹（File System Access API），
 * 在其下创建一个导出子目录，逐篇写入 .md 文件，落盘即可进 Obsidian vault
 * @param {Array<{filename, content}>} notes
 * @param {string} subFolderName - 在所选目录下创建的导出子目录名
 * @returns {Promise<{folderName: string, count: number}>}
 */
export async function saveObsidianNotesToDirectory(notes, subFolderName = 'anchor-read-export') {
  const rootHandle = await window.showDirectoryPicker({ mode: 'readwrite' });
  const dirHandle = await rootHandle.getDirectoryHandle(subFolderName, { create: true });
  const used = new Set();
  let count = 0;
  for (const note of notes) {
    const filename = dedupeFilename(used, note.filename);
  ...
  return { folderName: subFolderName, count };
}

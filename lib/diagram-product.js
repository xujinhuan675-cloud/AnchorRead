...
export const EXCALIDRAW_DEFAULT_VISUAL_CONTRACT = '除非用户明确指定视觉风格、颜色、质感或布局，否则使用 AnchorRead 默认视觉系统：3-4 种语义色、浅色实心填充、深色描边、统一圆角、背景分区、标题层级和充足留白；跨区连线使用带 elbowed=true 的多点 points 绕开节点。';
...
export function createDiagramGenerationPlan({ scope, content = '', intent = 'auto' } = {}) {
  ...
  if (normalizedScope === DIAGRAM_SCOPES.articleDeep) {
    return {
      ...
      renderer,
      prompt: '请对全文进行深度图解：先提取角色、阶段、关键步骤、关系和例外路径，再按清晰的视觉分组与层级生成一张可编辑图解。控制单个节点文案长度，保留关键细节；由系统负责最终画布排版。' + EXCALIDRAW_DEFAULT_VISUAL_CONTRACT,
    };
  }
...
export function switchDiagramVariant({
  ...
  const currentVariant = {
    ...(drawing?.variants?.[current] || {}),
    source: currentSource,
    chartType: normalizeDiagramIntent(currentChartType),
    updatedAt: now,
  };
  if (current === DIAGRAM_RENDERERS.excalidraw && currentScene) {
    currentVariant.scene = currentScene;
  }
  const variants = {
    ...(drawing?.variants || {}),

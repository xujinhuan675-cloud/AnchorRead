...
// UI persistence is allowed only for the tab that owns the local bridge lease.
// A background owner must remain writable so MCP work can finish while the
// user browses elsewhere. When no lease exists yet, the first page may write
// and the bridge will establish ownership immediately.

...
export const DIAGRAM_AGENT_LEASE_MS = 30_000;
...
export function shouldOwnDiagramAgentLease(
  ...
  // Hidden pages must not acquire or retain the browser writer lease. They can
  // still render a synchronized drawing, but persistence belongs to one
  // visible tab only.
...
// UI persistence is allowed only for the visible tab that owns the local
// bridge lease. When no lease exists yet (for example during first paint), a
// visible page may write and the bridge will establish ownership immediately.
export function canPersistDiagramDrawing({
  ...
  if (!isDiagramAgentPageVisible(documentRef)) return false;

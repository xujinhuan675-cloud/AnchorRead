...
export function shouldOwnDiagramAgentLease(
  ...
  // A background AnchorRead tab is still a valid IndexedDB writer. Ownership
  // changes only when the current page releases the lease or the lease expires.
...
export function createDiagramAgentSession({
  ...
  const acquire = () => {
    const timestamp = now();
    const current = read();
    if (!shouldOwnDiagramAgentLease(current, { tabId, now: timestamp })) {
      owner = false;
      return false;
    }
    if (!usableStorage) {

...
export function createInlineViewResult({ elements, title = 'Excalidraw', presentation = null } = {}) {
  ...
  const invalidCamera = parsed.find((element) => {
    if (element?.type !== 'cameraUpdate') return false;
    const width = Number(element.width);
    const height = Number(element.height);
    return width > 0 && height > 0 && Math.abs(width / height - 4 / 3) > 0.15;
  });
  ...
  return {
    ...
    ...(invalidCamera ? {
      qualityWarnings: [{
        code: 'CAMERA_ASPECT_RATIO',
        message: `cameraUpdate ${invalidCamera.width}x${invalidCamera.height} is not close to 4:3. Use 400x300, 600x450, 800x600, 1200x900, or 1600x1200.`,
      }],
    } : {}),
    openRequested: false,
    openAction: 'none',

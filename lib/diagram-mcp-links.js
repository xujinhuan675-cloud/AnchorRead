...
function normalizedOrigin(value = '') {
  ...
  try {
    ...
    // Container bind addresses are not browser-reachable public origins.
    if (url.hostname === '0.0.0.0' || url.hostname === '[::]') return '';
    return url.origin;
  } catch {
    return '';
  }
...
export function buildDiagramWakeUrl(requestId, { baseUrl = '', fallbackToDefault = true } = {}) {
  const workspaceUrl = buildDiagramWorkspaceUrl({ baseUrl, fallbackToDefault });
  const separator = workspaceUrl.includes('?') ? '&' : '?';
  return `${workspaceUrl}${separator}diagramWake=${encodeURIComponent(String(requestId || '').trim())}`;
}
...
export function createInlineDiagramResult(args = {}, error, { baseUrl = '' } = {}) {
  ...
  const recovery = {
    ...
    openAction: args.open !== false ? 'open_url_if_supported' : 'none',
    ...(args.open !== false ? { openTarget: 'default_browser' } : {}),
    openResource: { kind: 'workspace', url },
  };
  if (engine === 'mermaid') {
    return source ? {
...
export function createDeferredDiagramResult(args = {}, requestId, { baseUrl = '' } = {}) {
  const url = buildDiagramWakeUrl(requestId, { baseUrl });
  const inline = createInlineDiagramResult(args, null, { baseUrl }) || {};
  return {
    ...inline,
    title: inline.title || String(args.title || 'AnchorRead 图解').trim() || 'AnchorRead 图解',
    queued: true,
    requestId,
    nextAction: 'open_default_browser_and_persist_locally',
    url,
    openRequested: true,
    openAction: 'open_url_if_supported',
    openTarget: 'default_browser',
    openResource: { kind: 'workspace', url },
    note: '创建命令仅在服务端短时排队；默认浏览器打开后会自动写入该浏览器的 IndexedDB，服务端不长期保存图解 scene。',
  };
}

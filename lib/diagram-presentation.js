export const DIAGRAM_PRESENTATION_VERSION = 2;
export const DEFAULT_PRESENTATION_STEP_DURATION_MS = 1500;
export const DEFAULT_PRESENTATION_TRANSITION_MS = 600;
export const DEFAULT_PRESENTATION_REVEAL_MS = 360;
export const DEFAULT_PRESENTATION_HOLD_MS = 900;
export const PRESENTATION_PLAYBACK_RATES = Object.freeze([0.75, 1, 1.25, 1.5]);
...
export function normalizePresentationStep(step, index = 0) {
  ...
  return {
    ...
    durationMs: boundedMilliseconds(step.durationMs, DEFAULT_PRESENTATION_STEP_DURATION_MS),
    transitionMs: boundedMilliseconds(step.transitionMs, DEFAULT_PRESENTATION_TRANSITION_MS, 10_000),
    revealMs: boundedMilliseconds(step.revealMs, DEFAULT_PRESENTATION_REVEAL_MS, 10_000),
    holdMs: boundedMilliseconds(step.holdMs, DEFAULT_PRESENTATION_HOLD_MS),
...
/**
 * `durationMs` remains a backwards-compatible total-step hint, while reveal,
 * camera movement, and reading time are modeled independently. The longest
 * animation must finish before the human-readable hold begins.
 */
export function getPresentationStepPlaybackDuration(step, playbackRate = 1) {
  const durationMs = boundedMilliseconds(step?.durationMs, DEFAULT_PRESENTATION_STEP_DURATION_MS);
  const transitionMs = boundedMilliseconds(step?.transitionMs, DEFAULT_PRESENTATION_TRANSITION_MS, 10_000);
  const revealMs = boundedMilliseconds(step?.revealMs, DEFAULT_PRESENTATION_REVEAL_MS, 10_000);
  const holdMs = boundedMilliseconds(step?.holdMs, DEFAULT_PRESENTATION_HOLD_MS);
  const hasCameraMotion = Boolean(step?.camera || step?.focusElementIds?.length);
  const motionMs = Math.max(revealMs, hasCameraMotion ? transitionMs : 0);
  const safeRate = Math.max(0.5, Math.min(2, finiteNumber(playbackRate, 1)));
  return Math.round(Math.max(durationMs, motionMs + holdMs) / safeRate);
}

...
const CREATE_VIEW_TOOL = {
  ...
  description: '在当前对话中直接创建可编辑的 Excalidraw 画布。elements 必须是 JSON 数组字符串且数组顺序就是逐个显现顺序；默认保持稳定全局视口播放。流程图按“带 label 的节点、箭头、下一个带 label 的节点”交替排列，不要为节点文字另建 text 元素。可选 presentation.steps 用于共享的阶段播放；仅在跨阶段或跨区域时使用聚焦与镜头动画。不依赖浏览器工作区。',
  annotations: { readOnlyHint: true },
  inputSchema: {
    type: 'object',
    properties: {
      elements: { type: 'string', description: 'Excalidraw raw element 数组的 JSON 字符串。数组顺序就是绘制与播放顺序；节点文字应放入 shape.label。' },
      presentation: { type: 'object', description: '可选的阶段播放脚本；steps 支持 title、visibleElementIds、focusElementIds、highlightElementIds、camera、durationMs、transitionMs、revealMs、holdMs。默认保持全局视口，仅在跨阶段或跨区域时设置 focus/camera。' },
    },
    required: ['elements'],
    additionalProperties: false,
  },

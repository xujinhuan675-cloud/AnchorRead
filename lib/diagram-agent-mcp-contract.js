...
export const DIAGRAM_MCP_INSTRUCTIONS =
  'Call ensure_workspace_ready at the start of a persisted-diagram workflow, then call read_diagram_guide before creating or substantially editing a diagram. A local stdio companion may open the AnchorRead browser workspace automatically; a remote MCP returns a resource link for the client to open. Plan one flow direction and stable ids, create a coherent batch, and let create_diagram run the structural preflight. Unless the user explicitly specifies a visual style, use the default Excalidraw visual system: three or four semantic colors, light solid fills, dark strokes, consistent rounded cards, background zones, title hierarchy, and generous whitespace. Do not submit an all-white default-looking scene. After creation or a substantial edit, call verify_diagram (or describe_scene plus get_canvas_screenshot) and fix every reported issue before finishing. Use create_view for a pure in-chat editable Excalidraw canvas; it accepts the official elements JSON string and does not require a browser. Scene element order is render order, while presentation.steps is the authoritative playback order; the semantic playback order is source -> target -> connector. For every flow connector, provide a stable id, startElementId and endElementId (or Excalidraw start/end bindings), and put the relationship predicate in arrow.label.text instead of a standalone text element. Two-point straight connectors are allowed only for unobstructed single-in/single-out flow with a channel of at least 40px. Fan-in/fan-out, loops, cross-zone routes, obstacle crossings, or narrower channels must provide at least three relative points and elbowed=true; binding resolution preserves their intermediate waypoints and only updates the endpoints. If create_diagram rejects a CONNECTOR_ROUTING_RISK, repair the listed connector ids and retry rather than bypassing the preflight. Prefer generating presentation.steps with the diagram: reveal the source node, then the target node if it is not already visible, then the connector and its bound label; do not replay a node that is already visible as a new step. Each step should have a semantic title and cumulative visibleElementIds. When presentation is omitted, default playback infers this same source -> target -> connector order from bindings. Playback is human-paced and keeps a stable overview. Use create_diagram when a diagram should also be persisted in the AnchorRead browser workspace. Contentful Mermaid or Excalidraw diagrams automatically receive playback steps and start playback when opened. For incremental edits, use the AnchorRead diagram id, expectedRevision, and the diagram-scoped element tools so concurrent user changes fail instead of being overwritten.';
export const DIAGRAM_MCP_READ_ME =
  'Start with read_diagram_guide. Use create_view with a JSON string containing Excalidraw raw elements for a pure in-chat canvas, or create_diagram for a persisted AnchorRead diagram. Scene order controls rendering and presentation.steps controls playback. Keep element ids stable; bind every relationship connector with startElementId/endElementId (or native bindings), put node text in shape.label, and put concise relationship text in arrow.label.text. create_diagram performs a structural preflight; after creation or a substantial edit, use verify_diagram for the compact structure-plus-visual check, or call describe_scene and get_canvas_screenshot separately. Use align_elements and distribute_elements for repeated geometry. Use expectedRevision for mutations. The app accepts cameraUpdate/delete/restoreCheckpoint pseudo-elements while streaming and renders Mermaid source as SVG.';
...
const BASE_TOOLS = [
  ...
  },
  {
    ...
    description: '逐项验证 MCP OAuth、服务器签发的浏览器 bindingId、公开工作区 URL、浏览器 buildSha/buildVersion/protocolVersion、可写状态、IndexedDB 读取和完整往返；返回 PASS/FAIL 与可执行的 nextAction，不修改图解。',
    annotations: { readOnlyHint: true },
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
  },
  {
  ...
  {
    name: 'get_diagram',
    description: '默认读取轻量图解摘要。projection=full 或 include 显式请求时才返回 scene、source、history、variants 等大型字段。',
    inputSchema: {
      type: 'object',
      properties: {
        id: { type: 'string' },
  ...
  {
    name: 'verify_diagram',
    description: '完成一次图解验收：返回结构描述、质量预检和可选的当前画布截图。发现问题时给出可执行的修复建议。',
    annotations: { readOnlyHint: true },
    inputSchema: {
      type: 'object',
      properties: {
        id: { type: 'string' },
        visual: { type: 'boolean', description: '是否请求当前画布截图；无浏览器时会返回 visual unavailable。默认 true。' },
      },
      required: ['id'],
      additionalProperties: false,
    },
...
const WRITE_TOOLS = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        author: { type: 'string' },
        reason: { type: 'string' },
        operationId: { type: 'string', minLength: 1, description: 'Optional idempotency key; retrying the same request returns the original result without a new revision.' },
        retryOnConflict: { type: 'boolean', description: 'Re-read the latest revision and replay the stable element batch after an optimistic conflict.' },
  ...
  },
  {
    ...
    description: 'Atomically apply multiple element updates in one scene revision. Supports idempotent retries and optional conflict rebasing.',
    inputSchema: {
      type: 'object',
      properties: {
        id: { type: 'string', description: 'AnchorRead diagram id or routeId.' },
        updates: {
          type: 'array',
          minItems: 1,
          items: {
        ...
        expectedRevision: { type: 'number' },
        retryOnConflict: { type: 'boolean', description: 'Re-read the latest revision and retry when an optimistic conflict occurs.' },
        maxConflictRetries: { type: 'integer', minimum: 0, maximum: 5 },
        operationId: { type: 'string', minLength: 1, description: 'Optional idempotency key; retrying the same request returns the original result without a new revision.' },
        author: { type: 'string' },
        reason: { type: 'string' },
...
export function getDiagramMcpTools({ includeExport = false } = {}) {
  ...
  return [...BASE_TOOLS, ...exportTool, CREATE_VIEW_TOOL, CREATE_TOOL, ...CANVAS_TOOLS, ...WRITE_TOOLS].map((tool) => {
    if (!DIAGRAM_MCP_WRITE_TOOL_NAMES.has(tool.name)) return tool;
    return {
      ...tool,
      inputSchema: {
        ...tool.inputSchema,
        properties: {
          ...
          includeScene: { type: 'boolean', description: 'Include the complete scene in the mutation response. Defaults to false.' },
        },
      },
    };
  });

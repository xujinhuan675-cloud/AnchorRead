...
const BASE_TOOLS = [
  ...
  },
  {
    ...
    description: '逐项验证 MCP OAuth、公开工作区 URL、配对浏览器在线状态、IndexedDB 读取和完整往返；返回 PASS/FAIL 与可执行的 nextAction，不修改图解。',
...
const CREATE_TOOL = {
  ...
  description: '创建并保存一个新图解。浏览器在线时写入当前浏览器的本地 IndexedDB；open=true（默认）只请求 MCP 客户端在用户的默认浏览器中打开返回链接，不会强制当前标签页跳转。有内容的 Mermaid 或 Excalidraw 图解会自动生成播放步骤并开始播放，无需额外传 stream。浏览器暂时不可用时，只要传入 scene/elements/source，内容仍会直接渲染到当前对话画布。结果同时包含可打开的 resource_link。优先传入完整 Excalidraw scene 或 elements；也可传 Mermaid source。',
  inputSchema: {
    type: 'object',
    properties: {
...
const CREATE_VIEW_TOOL = {
  ...
  description: '在当前对话中直接创建可编辑的 Excalidraw 画布。elements 必须是 JSON 数组字符串且数组顺序就是绘制与播放顺序；流程图按“带 label 的节点、箭头、下一个带 label 的节点”交替排列，不要为节点文字另建 text 元素。可选 presentation.steps 用于共享的阶段播放、聚焦与镜头动画。不依赖浏览器工作区。',
  annotations: { readOnlyHint: true },
  inputSchema: {
    type: 'object',
    properties: {
      elements: { type: 'string', description: 'Excalidraw raw element 数组的 JSON 字符串。数组顺序就是绘制与播放顺序；节点文字应放入 shape.label。' },
      presentation: { type: 'object', description: '可选的阶段播放脚本；steps 支持 title、visibleElementIds、focusElementIds、highlightElementIds、camera、durationMs、transitionMs。' },
    },
    required: ['elements'],
    additionalProperties: false,
  },

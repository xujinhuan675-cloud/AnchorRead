...
export function createDiagramRevision({
  ...
  return {
    ...
    createdAt,
    ...(typeof operationId === 'string' && operationId.trim() ? { operationId: operationId.trim() } : {}),
    ...(typeof operationFingerprint === 'string' && operationFingerprint ? { operationFingerprint } : {}),
...
export function findDiagramRevisionByOperationId(drawing, operationId) {
  const target = String(operationId || '').trim();
  if (!target) return null;
  return listDiagramRevisions(drawing).find((item) => String(item.operationId || '') === target) || null;
}

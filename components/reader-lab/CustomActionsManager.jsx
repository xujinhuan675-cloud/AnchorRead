...
function ToggleSwitch({ checked, label, onToggle }) {
  return (
    <button
      ...
      aria-label={label}
      onClick={onToggle}
      className={`relative h-5 w-9 shrink-0 rounded-full transition-colors ${checked ? 'bg-gray-900' : 'bg-gray-200'}`}
    >
      <span
        aria-hidden="true"
      ...
      <div className="space-y-4">
        ...
        {actions.length === 0 && !formOpen ? (
          <div className="rounded-lg border border-dashed border-gray-300 p-8 text-center">
            ...
            <p className="mt-2 text-sm text-gray-400">浮动工具栏还没有动作，点击「新建动作」开始。</p>
          </div>
        ) : (
            ...
            {actions.map((action, index) => (
              <li key={action.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className={`truncate text-sm font-medium ${action.enabled === false ? 'text-gray-400' : 'text-gray-900'}`}>{action.name}</p>
                ...
                <div className="flex shrink-0 flex-col">
                  <button
                    ...
                    disabled={index === 0}
                    className="flex h-4 w-8 items-center justify-center rounded text-gray-500 hover:bg-gray-100 disabled:opacity-30"
                ...
                {action.builtin ? (
                  ...
                  <span className="flex h-8 w-8 items-center justify-center">
                    <span className="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] font-medium text-gray-500">内置</span>
                  </span>
                ) : (
                  <button
                    type="button"

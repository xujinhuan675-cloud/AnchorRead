    ...
    <Modal isOpen={isOpen} onClose={handleClose} title="术语表" maxWidth="max-w-3xl">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          type="button"
          onClick={startCreate}
          className="flex h-8 shrink-0 items-center gap-1.5 rounded border border-stone-200 dark:border-stone-800 px-2.5 text-[11px] font-medium text-stone-700 dark:text-stone-300 hover:bg-stone-50 dark:bg-white/5"
        >
      ...
      {formOpen ? (
        <form onSubmit={submit} className="mt-3 space-y-3 rounded border border-stone-200 dark:border-stone-800 bg-stone-50 dark:bg-white/5 p-3">
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block text-xs font-medium text-stone-600 dark:text-stone-400">
              ...
              <input
                ...
                placeholder="例如：幂等键"
                maxLength={100}
                className="mt-1 h-8 w-full rounded border border-stone-200 dark:border-stone-800 bg-white px-2.5 text-sm text-stone-900 dark:text-stone-100 outline-none focus:border-stone-400"
              />
            </label>
            <label className="block text-xs font-medium text-stone-600 dark:text-stone-400">
              别名（可选，逗号或顿号分隔）
          ...
          <div className="flex justify-end gap-2">
            <button
              ...
              className="h-8 rounded border border-stone-200 dark:border-stone-800 bg-white px-3 text-xs font-medium text-stone-600 dark:text-stone-400 hover:bg-stone-100 dark:bg-white/10"
            >
              取消
            </button>
            <button
              type="submit"
              className="h-8 rounded bg-stone-900 px-4 text-xs font-medium text-white hover:bg-stone-700 dark:bg-stone-300"
            >
              保存术语
            </button>
          </div>
      ...
      {entries.length === 0 && !formOpen ? (
        <div className="mt-3 rounded border border-dashed border-stone-200 dark:border-stone-800 bg-white p-6 text-center">
          <BookMarked size={20} className="mx-auto text-stone-300 dark:text-stone-600" aria-hidden="true" />
        ...
        <ul className="mt-3 space-y-3">
          {entries.map((entry) => (
            <li key={entry.id} className="rounded border border-stone-200 dark:border-stone-800 bg-white p-3 shadow-sm">
              <div className="flex items-center justify-between gap-2">
                <p className="min-w-0 truncate text-xs font-semibold text-stone-800 dark:text-stone-200">{entry.term}</p>
                <div className="flex shrink-0 items-center gap-1">
                  <button
                    type="button"
                    onClick={() => startEdit(entry)}
                    aria-label={`编辑术语"${entry.term}"`}
                    className="flex h-7 w-7 items-center justify-center rounded text-stone-500 hover:bg-stone-100 dark:bg-white/10"
                  >
              ...
              {entry.explanation ? (
                <p className="mt-2 break-words text-xs leading-5 text-stone-600 dark:text-stone-400">{entry.explanation}</p>
              ) : null}
              ...
              <p className="mt-2 text-[11px] text-stone-400">术语表条目 · AI 解读时沿用此定义，不再重复解释。</p>
            </li>
          ))}
        </ul>
      )}
